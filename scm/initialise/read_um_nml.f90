...
! *********************************COPYRIGHT*********************************
!
! Routine to read in SIZES control file generated by the UMUI. The namelists
! NLSIZES and STSHCOMP contain lots of variables that are not applicable to
! the SCM. To use the same namelists the UM generates without keeping the
! extra fields, the NLSIZES and STSHCOMP nameslists are read in this routine
...
SUBROUTINE read_um_nml                                                      &
  ( filename, rows, row_length, model_levels, wet_levels, cloud_levels      &
  , bl_levels, ozone_levels, st_levels, sm_levels, tr_levels                &
  , tr_vars, tr_ukca, nice )
...
! Local
  ...
  ! Logic for setting soil mositure levels - same as temp levels for
  ! surface hydrology 7A or 8A, otherwise zero
  sm_levels=0
  ...
  ! elements of TCA (which has been read from STSHCOMP above)
  tr_vars=0
  ...
  ! Logic for setting the num of ukca tracers - count the non-zero
  ! elements of TC_UKCA (which has been read from STSHCOMP above)
  TR_UKCA=0
