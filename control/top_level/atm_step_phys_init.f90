...
cfl_star,   cff_star, rho_km, ch, exner_lbc_real_tend, w_lbc_real_tend, &
errorstatus, flag )
...
USE rad_input_mod, ONLY:                                                &
    l_use_seasalt_indirect,l_use_seasalt_direct, i_ozone_int,           &
    l_use_cariolle,  l_use_ozoneinrad, lexpand_ozone
...
USE um_input_control_mod,  ONLY:                                        &
     model_domain,         problem_number,   l_dms,                     &
     l_dms_em,             l_dms_em_inter,   l_dms_ointer,              &
...
REAL :: rho_km(rkmdims%i_start:rkmdims%i_end,                          &
               rkmdims%j_start:rkmdims%j_end,                          &
               rkmdims%k_start:rkmdims%k_end)
REAL :: cH    (chdims %i_start:chdims %i_end,                          &
...
IF (lhook) CALL dr_hook('ATM_STEP_PHYS_INIT',zhook_in,zhook_handle)
...
! Inclusion of friction terms

  ! zero ch term
  DO k=chdims%k_start, chdims%k_end
    DO j=chdims%j_start, chdims%j_end
      DO i=chdims%i_start, chdims%i_end
        ch(i,j,k) = 0.
      END DO
    END DO
  END DO
  ...
  ! zero rho_km term
  DO k=rkmdims%k_start, rkmdims%k_end
    DO j=rkmdims%j_start, rkmdims%j_end
      DO i=rkmdims%i_start, rkmdims%i_end
        rho_km(i,j,k) = 0.
      END DO
    END DO
  END DO
...
dim_cs2=land_field
  ...
  IF (l_mcr_qcf2) THEN  ! Allocate second cloud ice
    ALLOCATE ( qcf2_star(qdims_s%i_start:qdims_s%i_end,              &
                         qdims_s%j_start:qdims_s%j_end,              &
                         qdims_s%k_start:qdims_s%k_end) )
  ELSE
  ...
  IF (l_mcr_qrain) THEN  ! Allocate rain
    ALLOCATE ( qrain_star(qdims_s%i_start:qdims_s%i_end,             &
                          qdims_s%j_start:qdims_s%j_end,             &
                          qdims_s%k_start:qdims_s%k_end) )
  ELSE
  ...
  IF (l_mcr_qgraup) THEN  ! Allocate graupel
    ALLOCATE ( qgraup_star(qdims_s%i_start:qdims_s%i_end,            &
                           qdims_s%j_start:qdims_s%j_end,            &
                           qdims_s%k_start:qdims_s%k_end) )
  ELSE
...
ELSE IF (flag == 'tropinit') THEN
  ...
  IF (lexpand_ozone) THEN
    nd_o3=rows*(o3dims2%k_end-o3dims2%k_start+1)
  ELSE
    nd_o3=row_length*rows*(o3dims2%k_end-o3dims2%k_start+1)
  END IF
...
IF (lhook) CALL dr_hook('ATM_STEP_PHYS_INIT',zhook_out,zhook_handle)
RETURN
