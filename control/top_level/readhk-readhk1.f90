...
!  Purpose: Read the operational model houskeeping file and set up
!           operational model control variables
...
SUBROUTINE readhk(unithk,icode,cmessage)
...
CHARACTER(LEN=*) :: cmessage ! Out - Return message if failure occured
!
...
CHARACTER(LEN=4)              :: type             ! Type of run
                                                  ! 'NRUN' -  New run
                                                  ! 'CRUN' -  Continuation
                                                  ! 'RRUN' -  Complete rerun
...
IF (lhook) CALL dr_hook('READHK',zhook_in,zhook_handle)

! 1. Open rewind and read record

CALL get_file(unithk,filename,filenamelength,icode)
OPEN(unithk,file=filename,status='old',IOSTAT=ICODE)

! Check for error opening file

IF (icode  >  0) THEN
  WRITE(6,*)'READHK : Failed in OPEN of input unit:'
  WRITE(6,*) filename
...
REWIND(unithk)
READ(unithk,'(I4,5I2,A4)',IOSTAT=ICODE) year,month,day,hour,indic,itype,type

! Check for error reading from file
...
! DEPENDS ON: time2sec
CALL time2sec(year,month,day,hour,0,0,0,0,iday1,isec1,lcal360)
! 
...
! 2. Set appropriate history variables
...
CLOSE(unithk)
IF (lhook) CALL dr_hook('READHK',zhook_out,zhook_handle)
RETURN
