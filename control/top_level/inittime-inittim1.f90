...
! ARGDUMA end
  ...
  USE um_input_control_mod, ONLY: lcal360, a_assim_start_min, a_assim_end_min
...
! Compute model_analysis_hrs from model_analysis_mins
  ...
  IF (newrun) THEN
    IF (model_assim_mode == "Atmosphere" .AND.                     &
       run_assim_mode == "Atmosphere")  THEN
      DO i=1,6
        model_basis_time(i)=a_fixhd(27+i)
      END DO
    ELSE IF ( .NOT.                                                 &
       (model_assim_mode == "Atmosphere" .AND.                     &
       run_assim_mode == "Atmosphere")                            &
       .AND.zerobasis) THEN
      DO i=1,6
        model_basis_time(i)=a_fixhd(27+i)
      END DO
    ...
    END DO
    !  Initialise job counter if first time-step
  ...
  ELSE IF (.NOT.newrun .AND. model_basis_time(2) == 0               &
    ...
    IF (h_stepim(a_im) >= model_analysis_hrs*a_steps_per_hr) THEN
      ...
      WRITE(6,'(A,F10.5,I10)') 'INITTIME; analysis_hrs,incr_secs ', & 
         model_analysis_hrs, increment_secs
    ELSE
      DO i = 1,6
        model_basis_time(i) = model_data_time(i)
      END DO
    END IF
    WRITE(6,'(A,6(I10,1x))')' INITTIME; model_data_time= ',model_data_time
    WRITE(6,'(A,6(I10,1x))')' INITTIME; model_basis_time= ',model_basis_time
  END IF
  !
  ...
  DO ii=1,n_internal_model
    ...
    IF ((elapsed_days /= 0.OR.elapsed_secs /= 0).AND.                 &
      ...
      WRITE(6,'(A)')cmessage
      WRITE(6,'(A,I10,A,I10,A,I10)')'internal model id',im,' old=',h_step, & 
         ' New=',step
    END IF
  ...
  ELSE
    ...
    run_job_counter = run_job_counter + 1
  ...
  IF (MOD(elapsed_secs,secs_per_period/steps_per_period) /= 0)THEN
    ...
    WRITE(6,'(A)')'INITTIME: Run length does not divide into timesteps'
    WRITE(6,'(A,I10,A,I10,A)') 'run length ',elapsed_days,' days ', &
       elapsed_secs,' seconds '
    WRITE(6,'(A,I10)')'TIMESTEP ',secs_per_period/steps_per_period
    WRITE(6,'(A)')'Modify run_target_end and resubmit'
    GO TO 9999
  END IF
  !
  DO ii=1,n_internal_model
