...
! This file belongs in section: Top Level
  ...
  USE nlstgen_mod, ONLY: pp_len2_meanim, ft_meanim
  USE Submodel_Mod 
...
! TYPDUMA end
  ...
  CHARACTER(len=filenamelength) ,INTENT(OUT) ::ppname    
                                            ! PP filename generated by GET_NAME
...
!
  ...
  IF (lhook) CALL dr_hook('PPCTL_INIT_CLIMATE_MEANS',zhook_in,zhook_handle)
  smid=i_ao
  IF (smid == a_im) THEN
    nftunit = ft_meanim(a_im)
  ELSE
    WRITE(6,'(A)')'PPCTL_INIT_CLIMATE_MEANS: UM only supports Atmos model'
    icode = 1
    Go To 9999
  END IF
...
!  1.2 Open named file on unit NFTUNIT
  ...
  WRITE(6,'(A,A,A,I3)')'PPCTL_INIT_CLIMATE_MEANS: Opening new file ',   &
      TRIM(ppname),' on unit ',nftunit
  len_ppname=LEN(ppname)

  CALL model_file_open(nftunit,ppname,len_ppname,FileNameProvidedExplicitly,  &
     OpenReadWrite,icode)
  IF (icode /= 0) THEN
...
!  1.3 Initialise the direct access LOOKUP headers (OUTPUT file)
    IF (smid == a_im) THEN
    WRITE(6,'(A,I3)')                                     &
       'PPCTL_INIT_CLIMATE_MEANS: Initialising new file on unit ',nftunit
    ! DEPENDS ON: init_pp
...
9999 CONTINUE
  IF (lhook) CALL dr_hook('PPCTL_INIT_CLIMATE_MEANS',zhook_out,zhook_handle)
  RETURN
