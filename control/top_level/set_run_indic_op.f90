...
!
      IF (lhook) CALL dr_hook('SET_RUN_INDIC_OP',zhook_in,zhook_handle)
      A_FIXHD(6) = RUN_INDIC_OP
      
!  new code specifically for Rose. Once migrated to Rose this
!  becomes the default for all model runs and above run_indic_op retired.
      
      IF (itab /= imdi) A_FIXHD(6) = itab       
!
      IF (lhook) CALL dr_hook('SET_RUN_INDIC_OP',zhook_out,zhook_handle)
      RETURN
