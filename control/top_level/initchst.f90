...
!  purpose: to initialise history file namelist variables
!
...
! Set common block area to zero or blank

! for NLIHISTO - integer overall model variables
  IF (lhook) CALL dr_hook('INITCHST',zhook_in,zhook_handle)
  DO  i = 1,6
...
! for NLCHISTG - character generic model variables
  ...
  DO  i = 1,n_internal_model_max
    checkpoint_dump_im(i)= blank_file_name
    save_dumpname_im(i)  = blank_file_name
  END DO

  DO  i = 1,nunits
    model_ft_unit(I) = blank_file_name
  END DO

  IF (lhook) CALL dr_hook('INITCHST',zhook_out,zhook_handle)
  RETURN
