...
! TYPINFA end
  ...
  CHARACTER(LEN=80), INTENT(  OUT)    :: cmessage ! Return message if failure 
                                                  ! occurred
  !
  !----------------------------------------------------------------------
  !  Common blocks
...
!
  ...
  !  Local variables
  !
  INTEGER            :: len_oldppfile   ! Length of old pp filename 
  ...
  CHARACTER(len=20)             :: local_tc  ! Local version of time_convention
  CHARACTER(len=filenamelength) :: ppname    ! PP filename
  !
  CHARACTER(len=filenamelength)  :: filename         
  !                                  Used to hold the filename of the
  ...
  IF (lhook) CALL dr_hook('PPCTL_REINIT',zhook_in,zhook_handle)
  IF (i_ao == a_im) THEN
    secs_per_step = secs_per_stePim(a_im)
    step=stePim(a_im)
  ELSE
    WRITE(6,'(A)')'PPCTL_REINIT: UM only supports Atmos model'
  END IF
  ...
  DO nftunit=20,nunits
    ...
    IF (lpp_select(nftunit)) THEN
      ...
      ! 
      oldppfile=model_ft_unit(nftunit)
      ...
      IF (ft_select(nftunit) == 'Y' .OR. ft_archsel(nftunit) == 'Y') THEN
        ...
        IF (oldppfile /= blank_file_name) THEN
      ...
      ELSE
        WRITE(6,'(A,I4,A,A)')                                                 &
            ...
            nftunit,' name=',TRIM(model_ft_unit(nftunit))
      END IF
          ...
          IF (nftunit >= 60  .AND. nftunit <= 69 .OR. nftunit == 151) THEN
            IF (ft_archsel(nftunit) == 'Y') THEN
              CALL jobCntrl(jc_archiveppnochart,oldppfile)
            END IF
          ELSE
            ! Archive all other selected unit numbers
            IF (type_letter_1(nftunit) == 'b') THEN
              CALL jobCntrl(jc_archivebndy,oldppfile)
            ELSE
              CALL jobCntrl(jc_archiveppnochart,oldppfile)
            END IF
          END IF
      ...
      IF (oldppfile /= blank_file_name) THEN
        IF (ft_select(nftunit) == 'Y') THEN
          CALL jobCntrl(jc_delete,oldppfile)
        END IF
      END IF
      ...
      IF (model_file_managed(nftunit)) THEN
        CALL model_file_close(nftunit,oldppfile,len_oldppfile,                &
            filenameProvidedExplicitly, doNotDeleteFile,icode)
      ELSE
        WRITE(6,'(A,I4,A,A)')                                                 &
            'PPCTL_REINIT: Warning tried to close unopen file on unit ',      &
            nftunit,' name=',oldppfile
      END IF


      WRITE(6,'(A,A,A,I3)')'PPCTL_REINIT: Opening new file ',  &
         TRIM(ppname),' on unit ',nftunit
      len_ppname=LEN(ppname)

      CALL Model_File_Open(nftunit,ppname,len_ppname,OpenReadWrite, &
          FileNameProvidedExplicitly,icode,allowRemap=.TRUE.,       &
      ...
      !      new filename
      ! 
! Store name of new file in namelist variable
      ...
      ELSE IF (type_letter_1(nftunit) == 'b') THEN
        ...
        IF (ft_output(nftunit) == 'Y') THEN
          WRITE(6,'(A,I3)')                                           &
              'PPCTL_REINIT: Initialising new boundary file on unit', &
              nftunit
...
! ARGINFA end
            ...
            IF (icode >  0) THEN
              WRITE (6,'(A)') ' PPCTL_REINIT : Error in IN_INTF - Atmos.'
              go to 9999  !  Return
            END IF
...
9999 CONTINUE
  IF (lhook) CALL dr_hook('PPCTL_REINIT',zhook_out,zhook_handle)
  RETURN
