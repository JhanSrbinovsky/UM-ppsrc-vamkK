...
! TYPPTRA end
      ...
      CHARACTER(LEN=80) CMESSAGE     ! Out - Error message on failure
!*----------------------------------------------------------------------
!  Common blocks
!
! --------------------- Comdeck: CHISTORY ----------------------------
...
!
      ...
      INTEGER IERR    !Return code from ABCALC
      INTEGER NQ      ! number of water variables in interface files 
...
!- End of comdeck CNAMINFA
        ...
        IF (lhook) CALL dr_hook('INTF_CTL',zhook_in,zhook_handle)
        ICODE=0
        CMESSAGE=' '
...
!     Initialise namelist parameters to cater for no boundary datasets
      DO JINTF = 1,MAX_N_INTF_A
        ...
        INTF_AVOID_ROT(JINTF)=.FALSE.
        ! Vertical interpolation is linear with extrapolation by default.
        INTF_V_INT_ORDER(JINTF)=interp_order_linear
        INTF_HORZGRID(JINTF)=' '
        INTF_VERTLEVS(JINTF)=' '
        INTF_L_EG_OUT=.false.             ! Set to true to quickly test code.
...
!  Read namelist for interface
      ...
      DO jintf = 1, n_intf_a
        IF ( Intf_row_length(jintf) > max_intf_lbcrow_length) THEN
          errorstatus = 15
          WRITE(cmessage, '(A,I3)') &
            'intf_row_length is larger than maximum allowed for area ', jintf
          CALL ereport( routinename, errorstatus, cmessage )  

        END IF

        IF ( Intf_p_rows(jintf) > max_intf_lbcrows) THEN
          errorStatus = 16
          WRITE(cmessage, '(A,I3)') &
            'intf_p_rows is larger than maximum allowed for area ', jintf
          CALL ereport( routinename, errorstatus, cmessage )  
        END IF
  
        IF ( Intf_p_levels(jintf) > max_intf_levels) THEN
          errorStatus = 17
          WRITE(cmessage, '(A,I3)') &
            'intf_p_levels is larger than maximum allowed for area ', jintf
          CALL ereport( routinename, errorstatus, cmessage )  
        END IF
...
! Read LBC VarGrid namelist 
          READ (UNIT=nft_HorzGrid, NML=horizgrid, IOSTAT=ErrorStatus) 
          CALL check_iostat(errorstatus, "namelist horizgrid")
          
! Write out namelist for diagnostic  
          If (PrintStatus >= PrStatus_Normal) THEN  
            WRITE ( UNIT = 6, NML = horizgrid) 
          End If
          
          DO j_lbc = 1, intf_row_length(jintf)
            Lambda_intf_p(J_lbc,jintf)= lambda_input_p(j_lbc)
            Lambda_intf_u(J_lbc,jintf)= lambda_input_u(j_lbc)
          END DO
          IF (intf_l_eg_out(jintf)) THEN
            DO j_lbc = 1, intf_p_rows(jintf)
              Phi_intf_p(J_lbc,jintf)= phi_input_p(j_lbc)
            END DO
            DO j_lbc = 1, intf_p_rows(jintf) + 1
              Phi_intf_v(J_lbc,jintf)= phi_input_v(j_lbc)
            END DO
          ELSE
            DO j_lbc = 1, intf_p_rows(jintf)
              Phi_intf_p(J_lbc,jintf)= phi_input_p(j_lbc)
...
! Check grid definition matches namelist sizes
          IF (lambda_input_p(intf_row_length(jintf)+1) /= RMDI   .OR.   &
              lambda_input_p(intf_row_length(jintf)  ) == RMDI   .OR.   &
              phi_input_p(intf_p_rows(jintf)+1)  /= RMDI         .OR.   &
              phi_input_p(intf_p_rows(jintf))    == RMDI)  THEN
              WRITE(cmessage, '(A)') 'Dimensions of &HORIZGRID namelist'&
                  // ' do not match those in &INTFCNSTA'
              errorstatus = 22
              CALL ereport(routinename, errorstatus, cmessage)
          END IF
        ...
        Else     ! Regular Grid LBC as UNUI input 
          ...
          If (PrintStatus >= PrStatus_Normal) Then    
            WRITE (6,'(A,I2)')                                          &
              ' Regular Grid for LBC Area: ', lbc_stream_a(jintf)
          End If
          IF (intf_l_eg_out(jintf)) THEN
            do J_lbc = 1, INTF_ROW_LENGTH(jintf)
      ...
      If (PrintStatus >= PrStatus_Normal ) THEN
        write (6,*) ' '
        Do jintf =1, n_intf_a
          WRITE (6,'(A,I2,A,A)') 'LBC Area ',lbc_stream_a(jintf),' : ', &
                                 TRIM(intf_vertlevs(jintf))
        End Do
      End If
...
! as wish to do this on *all* PEs together
        ...
        IF ( .NOT. l_exist ) THEN
          WRITE (6,'(A,A)') ' LBC Vertical Levels file: ',filename
          errorstatus = 20
...
! Read VERTLEVS namelist
        READ (UNIT=nft_vertlevs, NML=vertlevs, IOSTAT=ErrorStatus) 
        CALL check_iostat(errorstatus, "namelist vertlevs")
...
!L Initialise LBC_UNIT_NO_A and ft_output.
      ...
      WRITE (6,'(A)') ' '
      WRITE (6,'(A)') 'LBC_STREAM_A and LBC_UNIT_NO_A'
      DO jintf=1,n_intf_a
        WRITE (6,'(I2,A,I2,A,I3)') jintf,' stream no ',lbc_stream_a(jintf),  &
                                   ' unit_no ',lbc_unit_no_a(jintf)
      END DO
      WRITE (6,'(A)') ' '
      WRITE (6,'(A)') 'FT_Output for Boundary Files.'
      DO j=140,140+max_n_intf_a-1
        WRITE (6,'(A,I3,A,A)') ' Unit No ',J,' FT_Output ',ft_output(j)
      ENDDO
      ...
      DO jintf = 1,n_intf_a
        ...
        IF (intf_pack(jintf) <  0 .OR. intf_pack(jintf) >  2) THEN
          ...
          icode = 1
          WRITE (6,'(A,I2)') ' INTF_PACK ',intf_pack
          WRITE (6,'(A)') ' Valid values for INTF_PACK:'
          WRITE (6,'(A)') '   0 = no packing'
          WRITE (6,'(A)') '   1 = 32 bit packing'
          WRITE (6,'(A)') '   2 = pack according to STASHmaster DumpP value'
          GO TO 9999   !  Return
        END IF
...
! DEPENDS ON: intf_new_files
 ...
 9999 CONTINUE
      IF (lhook) CALL dr_hook('INTF_CTL',zhook_out,zhook_handle)
      RETURN
