...
IF (lhook) CALL dr_hook('ATM_STEP_STASH',zhook_in,zhook_handle)
IF (flag == 1) THEN
...
! ARGSTS end
  ...
  END IF ! .NOT. ( (Model_domain==mt_global ) etc
...
!Note [j,i]_start,, [j,i]_stop are set in atm_step_init
      DO k=            1, qdims%k_end
        DO j = j_start, j_stop
          DO i = i_start, i_stop
            inc_q(i,j,k)   = q(i,j,k)
    ...
    IF (l_mcr_qrain .AND. sf(189,30) ) THEN   ! extra prognostic rain
      ...
      DO k=            1, qdims%k_end
        DO j = j_start, j_stop
          DO i = i_start, i_stop
            inc_qrain(i,j,k)   = qrain(i,j,k)
          END DO
        END DO
      END DO
    ELSE
      ALLOCATE ( inc_qrain(1,1,1) )
    END IF        ! (l_mcr_qrain)

    IF (l_mcr_qgraup .AND. sf(190,30) ) THEN   ! extra prognostic graupel
      ...
      DO k=            1, qdims%k_end
        DO j = j_start, j_stop
          DO i = i_start, i_stop
            inc_qgraup(i,j,k)   = qgraup(i,j,k)
          END DO
        END DO
      END DO
    ELSE
      ALLOCATE ( inc_qgraup(1,1,1) )
    END IF        ! (l_mcr_qgraup)

    IF (l_mcr_qcf2 .AND. sf(191,30) ) THEN   ! extra prognostic ice
      ...
      DO k=            1, qdims%k_end
        DO j = j_start, j_stop
          DO i = i_start, i_stop
            inc_qcf2(i,j,k)   = qcf2(i,j,k)
          END DO
        END DO
      END DO
    ELSE
      ALLOCATE ( inc_qcf2(1,1,1) )
    END IF        ! (l_mcr_qcf2)

  END IF       ! sf(0,30)

  ! Note other arrays required by routine update fields as well as section 30
  ! stash output so not under sf(0,30) test
  ...
  l_do_inc_vels = sf(185,30) .OR. sf(186,30) .OR. sf(187,30)  &
             .OR. sf(175,30) .OR. sf(176,30) .OR. sf(177,30)
...
! DEPENDS ON: stash
    CALL stash(a_sm,a_im,39,stashwork39,                          &
! ARGD1 start
      ! IN/OUT:Addressing of D1 & D1 array
     &  D1_ADDR,D1,LD1,ID1,                                             &
! ARGD1 end
...
END IF ! flag
IF (lhook) CALL dr_hook('ATM_STEP_STASH',zhook_out,zhook_handle)
RETURN
