...
!- End of header ------------------------------------------------------
      ...
      IF (lhook) CALL dr_hook('INITIAL',zhook_in,zhook_handle)
...
!L
      ...
      WRITE(6,'(A)') 'Fast i/o of radincs directly to core memory'
      END IF
!L
...
!L ------ (with extra copy for logical values)-----------
     &  SPD1(IXD1( 1)), SPD1(IXD1( 2)), SPD1(IXD1( 2)), SPD1(IXD1( 2)), &
        ...
        IF (ICODE  /=  0) THEN
          WRITE(6,'(A)') 'Failure in call to INITCTL'
...
!L --------------- Derived constants (atmosphere)--------
     ...
     &               submodel,ICODE,CMESSAGE)
        IF (ICODE  /=  0) THEN
          WRITE(6,'(A)') 'Failure in call to INITDUMP'
...
!L --------------- Dump headers (atmosphere)-------------
      ...
      A_SPDUM(A_IXDUM(13)),A_SPDUM(A_IXDUM(14)),a_ixsts, a_spsts,       &
         ICODE,CMESSAGE)
      IF (ICODE  /=  0) THEN
        WRITE(6,'(A)') 'Failure in call to SET_RUN_INDIC_OP'
...
!L --------------- Dump headers (atmosphere)-------------
     ...
     &                ICODE,CMESSAGE)
        IF (ICODE  /=  0) THEN
          WRITE(6,'(A)') 'Failure in call to INITHDRS'
...
!}CABLE 
     ...
     &          ICODE,CMESSAGE)
          IF (ICODE  /=  0) THEN
            WRITE(6,'(A)') 'Failure in call to DUMPCTL (Atmos)'
...
!L --------------- Dump headers (atmosphere)-------------
      ...
      A_SPDUM(A_IXDUM(13)),A_SPDUM(A_IXDUM(14)),a_ixsts, a_spsts,       &
           submodel,ICODE,CMESSAGE)
        IF (ICODE  /=  0) THEN
          WRITE(6,'(A)') 'Failure in call to INITMEAN for submodel ',       &
             ISM
...
!L --------------- Dump headers (atmosphere)-------------
      ...
      A_SPDUM(A_IXDUM(13)),A_SPDUM(A_IXDUM(14)),a_ixsts, a_spsts,       &
         submodel,ICODE,CMESSAGE)
        IF (ICODE  /=  0) THEN
          WRITE(6,'(A)') 'Failure in call to INITTIME'

          CALL Ereport(RoutineName,ICODE,Cmessage)
        END IF
!L
!L 4.2  Write up history file after successfully reading
...
! DEPENDS ON: temphist
          ...
          IF (ICODE  /=  0) THEN
            WRITE(6,'(A)') 'Failure in call to TEMPHIST'
...
!L --------------- Ancillary file arrays(atmosphere)-------------
     ...
     &           ICODE,CMESSAGE)
        IF (ICODE  /=  0) THEN
          WRITE(6,'(A)') 'Failure in call to INANCCTL'
...
!L 4.4  Set up control block for updating boundary fields
 ...
 100  CONTINUE  ! Return here if IN_BOUND has been called and there
      ...
      IF (l_lbc_coup .AND.                                              &
        ...
        IF (PrintStatus  >=  PrStatus_Normal) THEN
          WRITE(6,'(A,A,A,A,A,A,A,A,A,A,A,A)') 'LBC_COUP: ',  &
...
! DEPENDS ON: timer
        ...
        IF (mype  ==  0) THEN  ! Only do the check on the communication
                               ! file on PE 0
          WRITE(6,'(A,I10)') 'ms_ntimes in INITIAL ',ms_ntimes
          ...
          DO WHILE (not_enough_lbcs) ! loop until the communication
            ...
            IF (ICODE  /=  0) THEN ! Something went wrong with the OPEN
              WRITE(6,'(A,I10)') &
                 'Return code from OPEN of communication file ',&
                 'in INITIAL: ',ICODE
              ICODE=401
              WRITE(CMESSAGE,'(A)') 'INITIAL : OPEN failed for Unit 190'
            ...
            DO WHILE (read_more_values) ! loop while we need to read
              ...
              IF (ICODE  /=  0) THEN ! Problem with the READ...
                ...
                WRITE(6,'(A,I10)') 'ms : Return code from READ ',ICODE
                ...
                IF (len_wait_tot  >=  um_lbc_wait_max) THEN
                  ...
                  WRITE(6,'(A,A,I10,A)') 'ms: Maximum wait time reached ', &
                ...
                ELSE ! We've not exceeded the time limit
                  ...
                  WRITE(6,'(A,I10,A)') 'ms: Wait for ',um_lbc_wait, &
                     ' seconds and retry.'
                  ...
                  WRITE(6,'(A,I10,A)') 'ms : Total wait so far ',len_wait_tot, &
                     ' seconds.'
                ...
                END IF ! IF (len_wait_tot  >=  um_lbc_wait_max)
              ...
              ELSE ! the READ(190) was successful - now we must
                ...
                ELSEIF (lbc_ntimes  <   ms_ntimes) THEN
                  ...
                  WRITE(6,'(A,I10,A,I10,A)') 'ms : lbc_ntimes = ',lbc_ntimes, &
                     ' read in.'//                              &
                ...
                END IF ! Test for value of lbc_ntimes
...
! DEPENDS ON: timer
        ...
        CALL GC_IBCAST(100,1,0,nproc,info,ICODE) ! PE 0 broadcasts ICODE
                                                 ! to all processors
        IF (ICODE  /=  0) THEN
          WRITE(6,'(A)') 'INITIAL - Error detected in LBC coupling.'
          WRITE(6,'(A,I10)') 'ICODE: ',ICODE
          WRITE(6,'(A,I10)') 'CMESSAGE : ',CMESSAGE
...
!L --------------- Input boundary arrays(atmosphere)-------------
     ...
     &                   ICODE,CMESSAGE)
        IF (ICODE  /=  0) THEN
          WRITE(6,'(A)') 'Failure in call to IN_BOUND'
     ...
     &              ICODE,CMESSAGE)
        ...
        IF (Num_ALBCs == 2) THEN
          ...
          IF (ALBC_num == 1 .AND. STEPim(a_im) >= ALBC_SwapStep) THEN
            ...
            IF (PrintStatus >= PrStatus_Normal) THEN
              WRITE (6,'(A)') ''
              WRITE (6,'(A)') 'INITIAL: Swapping to 2nd atmos boundary file'
              WRITE (6,'(A)') ''
            END IF
...
!L --------------- Input boundary arrays(atmosphere)-------------
     ...
     &          ICODE,CMESSAGE)
      ...
      END IF ! IF (BOUNDARY_STEPSim(a_im)  /=  0)
...
!L --------------- Derived constants (atmosphere)--------
      ...
      IF (ICODE  /=  0) THEN
        WRITE(6,'(A)') 'INITIAL: Failure in call to SETCONA'
...
!}CABLE 
         ...
         ICODE,CMESSAGE)
        IF (ICODE  /=  0) THEN
          WRITE(6,'(A)') 'Failure in call to INTF_CTL'
...
! DEPENDS ON: initphys
      CALL INITPHYS(ICODE,CMESSAGE)
        IF (ICODE  /=  0) THEN
          WRITE(6,'(A)') 'Failure in call to INITPHYS'
...
!L --------------- Derived constants (atmosphere)--------
       A_SPCON(A_IXCON(1)),A_SPCON(A_IXCON(2)),A_SPCON(A_IXCON(3)),  &
        ...
        IF (ICODE  /=  0) THEN
          WRITE(6,'(A)') 'Failure in call to INIT_EMCORR'
...
!------------------------------------------------------------------------
      IF (model_domain == mt_lam) THEN
        ...
        CALL fill_external_halos(v, row_length, n_rows, model_levels,     &
                                 offx, offy)
        CALL fill_external_halos(theta, row_length, rows, model_levels,   &
                                 offx, offy)
...
!------------------------------------------------------------------------
      IF (model_domain == mt_lam) THEN
        ...
        CALL set_external_halos(qrain, row_length, rows, wet_levels,     &
                                halo_i, halo_j, 0.0) 
        IF (l_mcr_qgraup .AND. .NOT. l_mcr_qgraup_lbc)                   &
        CALL set_external_halos(qgraup, row_length, rows, wet_levels,    &
                                halo_i, halo_j, 0.0) 
        IF (l_mcr_qcf2 .AND. .NOT. l_mcr_qcf2_lbc)                       &
        CALL set_external_halos(qcf2 ,row_length, rows, wet_levels,      &
                                halo_i, halo_j, 0.0) 
      END IF
...
A_SPINF(A_IXINF( 8)),A_SPINF(A_IXINF( 9)),                                     &
        ...
        IF (ICODE  /=  0) THEN
          WRITE(6,'(A)') 'Failure in call to SETTSCTL'
...
A_SPINF(A_IXINF( 8)),A_SPINF(A_IXINF( 9)),                                     &
         submodel,ICODE,CMESSAGE)
        IF (ICODE  /=  0) THEN
          WRITE(6,'(A)') 'Failure in call to PPCTL'
...
!}CABLE 
         ...
         ICODE,CMESSAGE)
      IF (ICODE  /=  0) THEN
        WRITE(6,'(A)') 'Failure in call to IN_ACCTL'
...
!L --------------- Derived constants (atmosphere)--------
     ...
     & Dummyarg)
          ...
          IF (ICODE  /=  0) THEN
            WRITE(6,'(A)') 'Failure in call to INITDIAG'
...
!L --------------- Ancillary file arrays(atmosphere)-------------
     ...
     &                  ICODE,CMESSAGE)
      END IF
          IF (ICODE  /=  0) THEN
            WRITE(6,'(A)') 'Failure in call to UP_ANCIL'
...
!}CABLE 
             ...
             ICODE,CMESSAGE)
          IF (ICODE  /=  0) THEN
            WRITE(6,'(A)') 'Failure in call to INIT_VEG'

            CALL Ereport(RoutineName,ICODE,Cmessage)
          END IF
        ELSE
          WRITE(6,'(A)')'INITIAL; skip INIT_VEG, LAND_FIELD=0 for this PE'
        END IF
      END IF
!
!L
...
!}CABLE 
            ...
            ICODE,CMESSAGE)
          IF (ICODE  /=  0) THEN
            WRITE(6,'(A)') 'Failure in call to INIT_CNV'
...
A_SPINF(A_IXINF( 8)),A_SPINF(A_IXINF( 9)),                                     &
     &          submodel,ndustbin_in,ndustbin_out,ICODE,CMESSAGE)
          IF (ICODE  /=  0) THEN
            WRITE(6,'(A)') 'Failure in call to GEN_INTF'
...
! DEPENDS ON: operatormessage
      ...
      END IF
 999  CONTINUE
      IF (lhook) CALL dr_hook('INITIAL',zhook_out,zhook_handle)
      RETURN
!L----------------------------------------------------------------------
