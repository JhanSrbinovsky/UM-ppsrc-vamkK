...
! *****************************COPYRIGHT*******************************
!
! Initialises trigonometric and Coriolis dynamical arrays
!
! Subroutine Interface:
      SUBROUTINE set_trigs(                                             &
      ...
      USE var_grid_mod,     ONLY: glambda_p, phi_p
...
! Local variables.
      ...
      REAL  ::  latitude_rot(row_length, rows)
      REAL  ::  longitude_rot(row_length, rows)
      REAL  ::  true_latitude_b(row_length, rows)
                                             ! true latitude on B-grid
      REAL  ::  true_longitude_b(row_length,rows)
...
! -----------------------------------------
    ...
    IF (lhook) CALL dr_hook('SET_TRIGS',zhook_in,zhook_handle)
...
!  For LAMs only, Trigs may be set to equatorial values
      ...
      ELSE    ! non-trivial trigonometric settings
        ...
        IF (model_domain == mt_global) Then
          ...
          IF (at_extremity(PNorth)) Then
            ...
            ELSE !  variable resolution
              DO i = 1, row_length
                FV_cos_theta_latitude(i, rows) = (phi_p(i,rows) -       &
                                                  phi_p(i,rows-1)) / 8. 
              END DO
            END IF !  L_regular
          ...
          IF (at_extremity(PSouth)) Then
            ...
            ELSE !  variable resolution
              DO i = 1, row_length
                FV_cos_theta_latitude(i,1) = (phi_p(i,2)-phi_p(i,1))/8.
              END DO
            END IF !  L_regular
...
!!! Allocate arrays for rot_coeff_mod module (not used for global)
        ...
        END DO
            ...
            DO j = 1, rows
              DO i = 1, row_length
                gi = datastart(1) + i - 1
                sin_theta_longitude(i,j) = sin(base_lambda +            &
                                                (gi-1)*delta_lambda)
                cos_theta_longitude(i,j) = cos(base_lambda +            &
                                                (gi-1)*delta_lambda)
                sin_u_longitude(i,j) = sin(Base_lambda +                &
                                               (gi-.5)*delta_lambda)
                cos_u_longitude(i,j) = cos(Base_lambda +                &
                                               (gi-.5)*delta_lambda)
              END DO
            END DO
...
!!! Allocate arrays for dyn_coriolis_mod module
      IF (.NOT. ALLOCATED(f1_at_v)) THEN
        ALLOCATE (f1_at_v (1-offx: row_length+offx,1-offy:n_rows+offy ))
      END IF
      IF (.NOT. ALLOCATED(f2_at_u)) THEN
        ALLOCATE (f2_at_u (1-offx: row_length+offx,1-offy:rows+offy ))
      END IF
          ...
          DO j = 1, rows
            DO i = 1, row_length
              f2_at_u(i,j) = two_omega * (                              &
                                   cos_theta_latitude(i,j) * temp2 -    &
                                   sin_theta_latitude(i,j) * temp1 *    &
                                      cos_u_longitude(i,j) )
              f3_at_u(i,j) = two_omega * (                              &
                                   sin_theta_latitude(i,j) * temp2 +    &
...
! DEPENDS ON: swap_bounds_2d_mv
      ...
      IF (lhook) CALL dr_hook('SET_TRIGS',zhook_out,zhook_handle)
