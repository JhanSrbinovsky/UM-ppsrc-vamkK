...
! Error reporting
      ...
      IF (lhook) CALL dr_hook('RESETATM',zhook_in,zhook_handle)
      ICODE=0
      CMESSAGE=''
!
!L----------------------------------------------------------------------
...
END IF
       ...
       Do k = 1, model_levels+1
         Do ij = 1, (rows + 2*offy) * (row_length + 2* offx)
           d1(jp(k) +ij-1) =                                            &
             (d1(jexner_rho_levels(k) +ij-1)**recip_kappa) * p_zero
         End Do
       End Do
...
! DEPENDS ON: calc_exner_at_theta
       CALL Calc_Exner_at_theta(                                        &
        ...
        d1(jexner_theta_levels(tdims%k_start)), .TRUE.)
...
! DEPENDS ON: calc_p_star
       ...
       IF (lhook) CALL dr_hook('RESETATM',zhook_out,zhook_handle)
       RETURN
