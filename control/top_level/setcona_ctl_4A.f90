...
!----------------------------------------------------------------------
      ...
      IF (lhook) CALL dr_hook('SETCONA_CTL_4A',zhook_in,zhook_handle)
...
! DEPENDS ON: setcona_4A
     ...
     &              D1(jorog_grad_x),D1(jorog_grad_y),D1(jorog_unfilt), &
     ...
     &              A_FIXHD(fh_RowDepCStart),                           &
                    D1(jexner_theta_levels(tdims_s%k_start)),           &
                    D1(jp_theta_levels(tdims_s%k_start)),               &
     &              D1(jp(1)), D1(jpstar),                              &
! ARGLNDM Constants for physics routines
        land_index, land_ice_index, soil_index,                         &
! ARGLNDM end
...
!     level routines in a generic way.
      ...
      ! !!! NOTE AND BEWARE !!!
      ! Integer pointers passed as real variables
...
!     is a real array and it needs to be filled with a logical.
      ...
      IF (l_ctile) then
        CALL expand_from_mask(fland,d1(jfrac_land),                     & 
      ...
      IF (lhook) CALL dr_hook('SETCONA_CTL_4A',zhook_out,zhook_handle)
      RETURN
