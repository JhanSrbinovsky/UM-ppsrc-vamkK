...
! Routine : TEMPHIST -------------------------------------------------
!
! Purpose :Write current contents of history common block to temporary
...
SUBROUTINE temphist                                                            &
  ...
  INTEGER, INTENT(IN)           :: unithist ! Temporary history file unit
  INTEGER, INTENT(INOUT)        :: icode    ! Return code from routine
  CHARACTER(LEN=80)             :: cmessage ! Return message if failure occured
...
!    file
!
  IF (lhook) CALL dr_hook('TEMPHIST',zhook_in,zhook_handle)
  ...
  CALL get_file(unithist,filename_base,filenamelength,icode)
  ...
  IF (printstatus>=prstatus_oper) THEN
    WRITE(6,'(A,A)')'Temphist: Writing temp history as :',                     &
        TRIM(filename)
    WRITE(6,'(A,A)')'Temphist: Final destination       :',                     &
        TRIM(filename_base)
  END IF
  ...
  IF (icode <= 0) THEN
    ...
    WRITE(unithist,nlihisto,iostat=icode)
    CALL ereport('temphist',icode,                                             &
        'Write ERROR on history file for namelist NLIHISTO')

    WRITE(unithist,nlchisto,iostat=icode)
    CALL ereport('temphist',icode,                                             &
        'Write ERROR on history file for namelist NLCHISTO')

    WRITE(unithist,nlihistg,iostat=icode)
    CALL ereport('temphist',icode,                                             &
        'Write ERROR on history file for namelist NLIHISTG')

    WRITE(unithist,nlchistg,iostat=icode)
    CALL ereport('temphist',icode,                                             &
        'Write ERROR on history file for namelist NLCHISTG')

    WRITE(unithist,nlcfiles,iostat=icode)
    CALL ereport('temphist',icode,                                             &
        'Write ERROR on history file for namelist NLCFILES')
...
! Instigate copy from versioned name to final name.
  ...
  IF (lhook) CALL dr_hook('TEMPHIST',zhook_out,zhook_handle)
