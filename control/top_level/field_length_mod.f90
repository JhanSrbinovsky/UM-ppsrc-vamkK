...
! Local variables:
      ...
      IF (lhook) CALL dr_hook('FIELD_LENGTH_MOD:FIELD_LENGTH',zhook_in,zhook_handle)
      select case (grid_type)
        case (theta_points                                              &
          ...
          y = tdims%j_end
        case (u_points)
          x = udims%i_end - udims%i_start + 1
          y = udims%j_end
        case (v_points)
          x = vdims%i_end
          y = vdims%j_end - vdims%j_start + 1
        case (land_points)
          ...
          field_length = length
          IF (lhook) CALL dr_hook('FIELD_LENGTH_MOD:FIELD_LENGTH',zhook_out,zhook_handle)
          RETURN
        case (river_points)
          x = river_row_length
          y = river_rows
