...
! TYPINFA end
  ...
  CHARACTER(LEN=80) ,INTENT(OUT) ::cmessage ! Return message if failure occurred
  !
  !*--------------------------------------------------------------
  !  Common blocks
  !
...
!
  ...
  CHARACTER(len=17)             ::local_tc ! Local version of time_convention
  CHARACTER(len=filenamelength) ::ppname   ! PP filename
  ...
  ! Fifth arguments to File_Open and fourth to Model_file_close defines whether 
  ...
  ! Fifth argument to Model_file_close indicates whether file should be deleted
  ...
  REAL, PARAMETER :: AnalysisHours = 0.0

  INTEGER(kind=jpim), PARAMETER :: zhook_in  = 0
  INTEGER(kind=jpim), PARAMETER :: zhook_out = 1
  REAL(kind=jprb)               :: zhook_handle

  IF (lhook) CALL dr_hook('PPCTL_INIT',zhook_in,zhook_handle)
  ...
  IF (i_ao == a_im) THEN
    ...
    step=stePim(a_im)
  ELSE
    WRITE(6,'(A)')'PPCTL_INIT: UM only supports Atmos model'
  END IF
  ...
  DO nftunit=20,nunits
    ...
    IF (ft_active(nftunit) == 'Y') THEN
      ...
      IF (ft_steps(nftunit) == 0) THEN
        ! Fixed name file names from environment variable
        CALL fort_get_env(ft_environ(nftunit), len_ft_envir(nftunit),  &
           ppname,filenamelength,icode)
        IF (icode >  0) THEN
          WRITE(6,'(A,A)')                                                &
             'PPCTL_INIT: Error getting value of environment variable ',  &
             TRIM(ft_environ(nftunit))
          GOTO 9999
        END IF
      ELSE
        ! Reinitialised filenames from restart information
        ppname = model_ft_unit(nftunit)
      END IF

      ! DEPENDS ON: init_pp_crun
      CALL init_pp_crun(nftunit,ppname,    &
         len1_lookup,                      &
         pp_len2_look(nftunit),            &
         type_letter_1(nftunit))

      
      IF(ft_steps(nftunit) >  0) THEN
      IF (model_file_managed(nftunit)) THEN
        CALL model_file_close(nftunit,model_ft_unit(nftunit),     &
            LEN_TRIM(model_ft_unit(nftunit)),FileNameProvidedExplicitly,  &
            DoNotDeleteFile,icode)
      ELSE
        WRITE(6,'(A,I4,A,A)')                                                 &
            'PPCTL_INIT: Warning tried to close unopen file on unit ',        &
            nftunit,' name=',TRIM(ft_environ(nftunit))   
      END IF         
      END IF
    END IF
    ! 
    ...
    IF (lpp_select(nftunit)) THEN
      IF (model_file_managed(nftunit)) THEN
        CALL model_file_close(nftunit,ft_environ(nftunit), &
            len_ft_envir(nftunit),FileNameInEnvVar,        &
            DoNotDeleteFile,icode)
      ELSE
        WRITE(6,'(A,I4,A,A)')                                                 &
            'PPCTL_INIT: Warning tried to close unopen file on unit ',        &
      ...
      IF (ft_steps(nftunit) /= 0 .AND.         &
        ...
        WRITE(6,'(A,A,A,I3)')'PPCTL_INIT: Opening new file ', &
           TRIM(ppname),' on unit ', nftunit

        len_ppname=LEN_TRIM(ppname)

        CALL model_file_open(nftunit,ppname,len_ppname,OpenReadWrite, &
           FileNameProvidedExplicitly,fileType=ioFileTypeUM)

        ! 
        !  1.6 Update history file record for appropriate unit 
        !      with new filename
        ! 
        model_ft_unit(nftunit)=ppname
      END IF
      ...
      IF (type_letter_1(nftunit) == 'p' .OR. &
        ...
        IF (ft_output(nftunit) == 'Y'               &
          ...
          IF (smid == a_im) THEN
            ...
            IF(ft_steps(nftunit) == 0) THEN
              CALL model_file_open(nftunit,ft_environ(nftunit),                &
                 len_ft_envir(nftunit),OpenReadWrite,FileNameInEnvVar,         &
                 fileType=ioFileTypeUM)
            END IF
            WRITE(6,'(A,I4)')'PPCTL_INIT: Initialising new file on unit ', &
                nftunit
            ! DEPENDS ON: init_pp
      ...
      ELSE IF (type_letter_1(nftunit) == 'b') THEN
        ...
        IF (ft_output(nftunit) == 'Y') THEN
          WRITE(6,'(A,I4)')                                        &
              'PPCTL_INIT: Opening boundary output file on unit ', &
              nftunit
          ! 
          !  Open file if not to be re-initialised, 
          !  i.e. file name is in environment variable.
...
! ARGINFA end
            ...
            IF (icode >  0) THEN
              WRITE (6,'(A)') ' PPCTL_INIT: Error in IN_INTF - Atmos.'
              GOTO 9999  !  Return
            END IF
...
!      IF(ft_steps(nftunit) /= 0)THEN
!        len_ppname=LEN_TRIM(ppname)
!        CALL model_file_close(nftunit,ppname,len_ppname, &
!            FileNameProvidedExplicitly, &
!            DoNotDeleteFile,icode)
!      END IF
...
9999 CONTINUE
  IF (lhook) CALL dr_hook('PPCTL_INIT',zhook_out,zhook_handle)
  RETURN
