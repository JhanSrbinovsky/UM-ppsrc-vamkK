...
!                           control system; On-line document C2 -
!                           Dump Handling.
! 
!   -------------------------------------------------------------------
...
! ARGSTS end
  ...
  USE nlstgen_mod, ONLY: steps_per_periodim, secs_per_periodim,  &
      dumpfreqim, archdump_freqim, archdump_offsetim
  USE Submodel_Mod
...
! TYPSTS end
  ...
  ! MEANLEV is actually flag for analysis dump (-1). Or set to zero
  INTEGER MEANLEV          ! IN  - Mean period level for dump
  INTEGER writestep        ! IN  - Timestep on which to write it
  CHARACTER(LEN=filenamelength) :: tmpfname    
                           ! IN  - Name of file to be written
...
!  Local variables
  ...
  CHARACTER(LEN=1)   FILETYPE    ! Code letter for file type
  CHARACTER(LEN=1)   LETTER_3    ! dummy argument for GET_NAME
  CHARACTER(LEN=filenamelength) ::  dumpname  ! Model generated dump name
...
! ----------------------------------------------------------------------
  ...
  IF (lhook) CALL dr_hook('DUMPCTL',zhook_in,zhook_handle)
  ...
  IF (I_AO == 1) THEN
    ...
    END DO
  END IF
!  4. Assign dump name to appropriate IO unit and open for write.
!     NB: Analysis dumps have preassigned names
! 
...
!   Construct dump name from model information using defined
!   naming convention.
! 
  dumpname = ' '
  IF (MEANLEV /= -1) THEN             ! Analyses already named
    IF (lwritd1) THEN
      STEP = STEPim(I_AO)
      IF (STEP  /=  writestep) GO TO 9999
      WRITE(dumpname,'(A,I4.4)') '..........',step
      DO i=1,10
...
! DEPENDS ON: get_name
        CALL GET_NAME(expt_id,job_id,I_AO,MEANLEV,TOGGLE,             &
           REINIT_STEPS,FILETYPE,LETTER_3,model_status,               &
           time_convention,0.0,DUMPNAME,ICODE,CMESSAGE,LCAL360)
      END IF
      IF (ICODE >  0) GO TO 9999
    END IF
  ELSE
    ! Get name for atmosphere analysis dump from ATMANL env var 
    CALL fort_get_env(ft_environ(nftout), len_ft_envir(nftout),  &
       dumpname,filenamelength,icode)
    IF (icode >  0) THEN
      WRITE(6,'(A,A)')                                                &
         'PPCTL_INIT: Error getting value of environment variable ',  &
         TRIM(ft_environ(nftout))
      GO TO 9999
    END IF
  END IF
...
!  4.1 Open unit for dump : 

  WRITE(6,'(A,A,A,I3)')"DUMPCTL: Opening new file ",TRIM(dumpname), &
     " on unit ",nftout
  len_dumpname=LEN_TRIM(dumpname)

  CALL Model_File_Open(nftout,dumpname,len_dumpname,1,1,error,allowRemap=.TRUE.)
  IF (error /= 0) THEN
    icode = 1
  ...
  IF(mype == 0) THEN
    IF(PrintStatus >= PrStatus_Diag) THEN
      WRITE(6,'(A,A,A,I4,A,i10,A)')                             &
          'Dumpfile Size for File ',TRIM(dumpname),' on Unit ', &
          nftout,' to be set to ', disk_address,' Words'
    END IF
...
! (SynchroniseAll has completed)
  ...
  len_dumpname=LEN_TRIM(dumpname)        
  CALL Model_File_Close(nftout,dumpname,len_dumpname,1,0,icode)
...
! 5.2 Exit here for WRITD1 temporary writes of dumps since
!     no requests need to be sent to the archive server

  IF (lwritd1) GO TO 9999

! ----------------------------------------------------------------------
!  6. Construct dump processing requests and send to slave task
! 

!   Save dump name for writing to restart file on successful completion
!   of timestep
  checkpoint_dump_im(a_im)=dumpname

!
!  6.2 If current dump is to be archived construct archiving request
!      followed by delete request if appropriate
! 
  LARCHIVE = .FALSE.
    ...
    IF (lclimrealyr) THEN ! get freq + offset in terms of real
      ...
      IF (i_day  ==  1 .AND. i_hour  ==  0) THEN ! end of month
        ...
        archdump_monoffset=(dumpfreqim(I_AO)*archdump_offsetim(I_AO)&
           *secs_per_periodim(I_AO))/               &
           (30*86400*steps_per_periodim(I_AO))
!  N.B. i_month is used below, not (i_month-1), because offset 
!  is from _start_ of 1st month
...
!           Calculate whether this timestep is an archive time
  ...
  ELSE IF (MEANLEV <= 0 .AND. dumpfreqim(I_AO) == 0) THEN
...
!         frequency is greater than 0.
  ...
  IF (mype == 0) THEN
    IF (LARCHIVE) THEN
      !           Archiving turned on
      CALL jobCntrl(JC_archivedump,dumpname)
...
9999 CONTINUE
  ...
  IF (lhook) CALL dr_hook('DUMPCTL',zhook_out,zhook_handle)
  RETURN
