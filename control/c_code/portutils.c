...
# 17 "portutils.c" 2
# 1 "/plush/dugong/usr/bin/../lib/gcc/x86_64-redhat-linux/4.4.7/include/stdbool.h" 1 3 4
# 18 "portutils.c" 2
...
(integer * length){ *length=sizeof(real); }
...

{
  ...
  *err=0;
  ...
  if ( *unit < 100){
    strcpy (fname, "UNIT");
    sprintf(fno,"%02d", (int) *unit);
    strcat(fname,fno);
    fname[6]='\0';
  }
  else{
    strcpy (fname, "UNIT");
    sprintf(fno,"%03d", (int) *unit);
    strcat(fname,fno);
    fname[7]='\0';
  }
...
(char * object1,char * object2, integer * diff)
{
...
(char * dest,char * src, integer * n)
{
  memcpy((void*)dest,(void*)src,*n * sizeof(integer));
}
...
int um_str_split(char * str,char splitter, char *** strlist)
{
  ...
  *strlist = malloc (numargs*sizeof(char *));
  ...
  arg=0;
   for (i=0;i<=strlen(str);i++){
    if (str[i]==splitter || i==strlen(str)){
      ...
      (*strlist)[arg]=malloc(end-start);
      memcpy((*strlist)[arg],&str[start+1],end-start-1);
      (*strlist)[arg][end-start-1]='\0';
      start=i;
      arg++;
    }
  }
...
integer file_op_touch(char * touch){
  ...
  ret_code=open(touch,0100 | 0200 |0400 , 0400 |0200 );
  if (ret_code==-1){
...
integer file_op_rm(char * opt, char * in){
  ...
  char message[384];
  integer minus_one=-1;
  integer *the_unit=&minus_one;
  _Bool silent=0;

  if (strcmp(opt,"-f")==0) silent=1;
  ...
  if (unlink(in)==0){
    snprintf(message, 384 ,"file_op: Delete: File deleted");
    fprintf(stdout, "%s\n", message);
  }
  else{
    if ( ! ((access(in,0)!=0) && silent) ){
      perror("File Delete:");
      snprintf(message, 384 ,"file_op: Delete: Failed");
      fprintf(stdout, "%s\n", message);
      ret_code=1;
    }
  }
...
integer file_op_mkdir(char * in){
  ...
  fprintf(stdout, "%s\n", message);
  if (mkdir(in,0755)==0){
    snprintf(message, 384 ,"file_op: Create Directory: Directory Created");
    fprintf(stdout, "%s\n", message);
  }
  else{
 ...
 fprintf(stdout, "%s\n", message);
      ...
      ret_code=1;
      perror("mkdir:");
      snprintf(message, 384 ,"file_op: Create Directory: Failed.");
      fprintf(stdout, "%s\n", message);
    }
  }
  return ret_code;
...
(char * command,integer * command_len){
  char *fname;
  int i;
  integer minus_one=-1;
  integer *the_unit;
  ...
  if ( i == -1 ){
    ...
    snprintf(message, 384 , "C Error: failed in SHELL\n");
    fprintf(stdout, "%s\n", message);
  }
  free( fname );
}
...
(integer * unit, integer * length){return;}
