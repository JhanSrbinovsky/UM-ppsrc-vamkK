...
! TYPLNDM end
     ...
     &  ,ICODE     ! OUT - Return code from routine
      ...
      CHARACTER(LEN=80) CMESSAGE ! OUT - Return message if failure occurred
!
!*----------------------------------------------------------------------
!  Common blocks
!
...
!     Set to atmosphere internal model
      IF (lhook) CALL dr_hook('ST_MEAN',zhook_in,zhook_handle)
      im_ident = atmos_im
...
! Primary data: in
     & D1(JPSTAR),D1(JP(1)),D1(JRHO(1)),D1(JU(1)),D1(JV(1)),D1(JW(0))   &
      ,D1(JTHETA(tdims%k_start)),D1(JQ(qdims%k_start))                  &
      ,D1(JQCL(qdims%k_start)),D1(JQCF(qdims%k_start))                  &
      ,D1(jp_theta_levels(tdims%k_start))                               &
      ,D1(jexner_rho_levels(1)),D1(jexner_theta_levels(tdims%k_start))  &
! Grid sizes and definition: in
     &,rows,n_rows,row_length,model_levels,wet_levels,bl_levels         &
     &,theta_field_size,u_field_size,v_field_size                       &
...
! ARGSTS end
 ...
 9999 CONTINUE
      IF (lhook) CALL dr_hook('ST_MEAN',zhook_out,zhook_handle)
      RETURN
