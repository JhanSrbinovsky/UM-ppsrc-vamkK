...
!
!    Logical components covered: D713
!
!    Project task: D7
!
...
USE sterr_mod, ONLY: st_upper_less_lower, st_not_supported,       &
                     ...
                     st_bad_address, st_unknown, st_bad_wraparound,&
                     st_illegal_weight, unknown_weight,           &
                     unknown_mask, unknown_processing, nonsense
IMPLICIT NONE
...
INTEGER, INTENT(IN)  :: rmdi              ! missing data indicator
...
REAL, ALLOCATABLE :: partial_sumztop_2d(:,:)
REAL, ALLOCATABLE :: partial_sumzbot_2d(:,:)
REAL, ALLOCATABLE :: sumztop(:)
REAL, ALLOCATABLE :: sumzbot(:)

INTEGER(KIND=jpim), PARAMETER :: zhook_in  = 0 
INTEGER(KIND=jpim), PARAMETER :: zhook_out = 1
REAL(KIND=jprb)               :: zhook_handle

IF (lhook) CALL dr_hook('STZONM',zhook_in,zhook_handle)
...
! 2.2 Add do the actual sum
  ...
  ALLOCATE(partial_sumzbot_2d(local_sum_xstart:local_sum_xend,    &
                 local_sum_ystart:local_sum_yend))
  ALLOCATE(partial_sumztop_2d(local_sum_xstart:local_sum_xend,    &
                 local_sum_ystart:local_sum_yend))
  ALLOCATE(sumzbot(local_sum_ystart:local_sum_yend))
  ALLOCATE(sumztop(local_sum_ystart:local_sum_yend))

  partial_sumzbot_2d(:,:)=0.0
  partial_sumztop_2d(:,:)=0.0
  sumzbot(:)=0.0
  sumztop(:)=0.0
...
! Only do the sum if this is not a halo point
    ...
    DO j=local_sum_ystart,local_sum_yend
      IF (mask(ii,j)) THEN
          ...
          partial_sumzbot_2d(ii,j) = pstar_weight(ii,j)                 &
                                   * area_weight(ii,j)
          partial_sumztop_2d(ii,j) = fieldin(ii,j) * pstar_weight(ii,j) &
                                   * area_weight(ii,j)
...
IF ((local_sum_ystart  /=  st_no_data) .AND.                      &
  ...
  CALL global_2d_sums(partial_sumzbot_2d, partial_sum_data_sizex, &
                      1, 0, 0, partial_sum_data_sizey, sumzbot,   &
                      gc_proc_row_group)

  CALL global_2d_sums(partial_sumztop_2d, partial_sum_data_sizex, &
                      1, 0, 0, partial_sum_data_sizey, sumztop,   &
                      gc_proc_row_group)
...
!     the subdomain
  IF ((local_sum_xstart  /=  st_no_data) .AND.                    &
    ...
    DO j=local_sum_ystart,local_sum_yend
      ...
      IF (sumzbot(j)  ==  0.0) THEN
        fieldout(jj)=rmdi
      ELSE
        fieldout(jj)=sumztop(j)/sumzbot(j)
      END IF
    END DO

  END IF ! is this processor in the subdomain

  DEALLOCATE(partial_sumzbot_2d)
  DEALLOCATE(partial_sumztop_2d)
  DEALLOCATE(sumztop)
  DEALLOCATE(sumzbot)

END IF ! does the subdomain intersect with this processor row

IF (lhook) CALL dr_hook('STZONM',zhook_out,zhook_handle)
