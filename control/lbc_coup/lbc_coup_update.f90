...
! ----------------------------------------------------------------------
  ...
  IF (lhook) CALL dr_hook('LBC_COUP_UPDATE',zhook_in,zhook_handle)
...
161     CONTINUE
        ...
        IF (icode /= 0) THEN
           ...
           END IF
           ! We need the time in microseconds for um_sleep.
        ...
        ELSE IF (lbc_ntimes <  ms_ntimes) THEN
...
! ARGBND Control data calculated from NAMELIST-
  ...
  IF (lhook) CALL dr_hook('LBC_COUP_UPDATE',zhook_out,zhook_handle)
  RETURN
