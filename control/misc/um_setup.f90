...
! *****************************COPYRIGHT*******************************
!
!+ Reads History & control files; also HK for Op.
!
! Subroutine Interface:
      ...
      USE Submodel_Mod
      USE nlstcall_mod, ONLY : model_status
...
!
      ...
      IF (lhook) CALL dr_hook('UM_SETUP',zhook_in,zhook_handle)
...
!
! DEPENDS ON: readhist
      CALL READHIST ( XHIST_UNIT,ICODE,CMESSAGE )
      IF (ICODE  >   0) GO TO 9999

! ----------------------------------------------------------------------
!  Read Housekeeping file for operational runs only.
!
...
! ----------------------------------------------------------------------
!  Output any error/warning message and stop if error.
!
 9999 CONTINUE
...
! ----------------------------------------------------------------------
!  Transfer end of TIMER section to UM_INDEX to get full SETUP time
!
      IF (lhook) CALL dr_hook('UM_SETUP',zhook_out,zhook_handle)
      RETURN
