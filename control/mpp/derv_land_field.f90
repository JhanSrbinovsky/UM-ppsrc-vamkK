...
SUBROUTINE derv_land_field (icode,cmessage)
...
IF (lhook) CALL dr_hook('DERV_LAND_FIELD',zhook_in,zhook_handle)
...
CALL model_file_open (unit_no,ft_environ(unit_no),                      &
    len_ft_envir(unit_no),                                        &
    ioOpenReadOnly,ioNameInEnv,icode)

!     Check error code from file_open
IF (icode >  0) THEN
  WRITE (6,*) 'Error in FILE_OPEN called from DERV_LAND_FIELD.'
  WRITE (cmessage,*) 'DRLANDF1 : Error in opening:'//ft_environ(unit_no)
  GO TO 9999   !  Return
END IF
...
CALL model_file_close(unit_no,ft_environ(unit_no),                      &
    len_ft_envir(unit_no),ioNameInEnv)

 9999 CONTINUE

IF (lhook) CALL dr_hook('DERV_LAND_FIELD',zhook_out,zhook_handle)
RETURN
