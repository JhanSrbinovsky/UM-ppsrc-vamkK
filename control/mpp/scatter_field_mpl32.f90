...
IF (lhook) CALL dr_hook('SCATTER_FIELD_MPL32',zhook_in,zhook_handle)

! 0.0 Error checking.
! Do we recognise the field type?
IF (grid_type  ==  fld_type_unknown) THEN
  WRITE (6,*) 'SCATTER_FIELD_MPL32 : bad field type'
...
IF (lhook) CALL dr_hook('SCATTER_FIELD_MPL32',zhook_out,zhook_handle)
RETURN
