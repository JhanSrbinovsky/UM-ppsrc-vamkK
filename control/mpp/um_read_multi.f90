...
! Parallel UM interface to BUFFIN - for use by the UM only.
! This version can use MPI calls with no GCOM equivalent

! Subroutine Interface:
SUBROUTINE um_read_multi(nft,d1,isize,unpack_size,len_io,local_len,  &
                         lookup,fixhd12,                             &
                         addr_info,n_levels,                         &
                         icode,cmessage)
...
INTEGER, INTENT(INOUT) :: addr_info(d1_list_len)   ! addressing info about field
...
INTEGER, EXTERNAL :: get_fld_type  ! function
! ------------------------------------------------------------------

IF (lhook) CALL dr_hook('UM_READ_MULTI',zhook_in,zhook_handle)
len_io=isize
local_len=0
...
halo_code = addr_info(d1_halo_type)
...
IF (new_decomp  /=  orig_decomp) THEN
  ...
  IF (icode  /=  0) THEN
    WRITE(6,*) 'UM_READ_MULTI: Error changing to decomposition ', &
                new_decomp
    WRITE(6,*)                                                    &
...
!       Has the data been read in OK?
  IF ((buf_icode  /=  -1.0) .OR. (len_io  /=  isize)) THEN
    WRITE(6,*) 'UM_READ_MULTI: Error in call to BUFFIN'
...
! DEPENDS ON: read_unpack
  IF (grid_code /= ppx_atm_tall  .AND.                                 &
      grid_code /= ppx_atm_cuall .AND.                                 &
      grid_code /= ppx_atm_cvall) THEN
...
IF ((grid_code == ppx_atm_tall  .OR.         &
     grid_code == ppx_atm_cuall .OR.         &
     grid_code == ppx_atm_cvall ) .AND.      &
     MOD((lookup(lbpack)),10)  ==  2) THEN
...
! DEPENDS ON: scatter_field_mpl32
     ...
     local_len=addr_info(d1_length)/addr_info(d1_no_levels)
! DEPENDS ON: expand21
...
! DEPENDS ON: general_scatter_field
  ...
  IF (icode  ==  1) THEN
    WRITE(6,*) 'UM_READ_MULTI: Call to GENERAL_SCATTER_FIELD failed'
...
IF (lhook) CALL dr_hook('UM_READ_MULTI',zhook_out,zhook_handle)
RETURN
END SUBROUTINE um_read_multi
