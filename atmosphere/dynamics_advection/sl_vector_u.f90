...
IF (lhook) CALL dr_hook('SL_VECTOR_U',zhook_in,zhook_handle)
itype = 1  ! u points.
error_code=0
...
!$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) SHARED(lambda_a, r_u, yu_d,   &
!$OMP& yv_d, yw_d, depart_phi, depart_lambda, j_end, j_begin, row_length,      &
!$OMP& model_levels) PRIVATE(cos_lambda_ad, sin_lambda_ad, sin_phi_d,          &
!$OMP& cos_phi_d, i, j, k)
...
! End of routine.
IF (lhook) CALL dr_hook('SL_VECTOR_U',zhook_out,zhook_handle)
RETURN
