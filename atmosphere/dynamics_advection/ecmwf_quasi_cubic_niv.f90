...
! ----------------------------------------------------------------------
      ...
      IF (lhook) CALL dr_hook('ECMWF_QUASI_CUBIC_NIV',zhook_in,zhook_handle)
      one_sixth = 1./6.
! begin loop over levels which ends at end of subroutine

!$OMP PARALLEL DO SCHEDULE(STATIC,1) DEFAULT(NONE)                      &
!$OMP& PRIVATE(i,j,k,val_i_minus,val_i,val_i_plus,val_i_plus2,          &
!$OMP&         coeff_plus2,coeff_plus,coeff_zero,coeff_minus,           &
!$OMP&         phi_cubed,phi_sq,lambda_cubed,lambda_sq)                 &
!$OMP& SHARED(weight_phi,weight_lambda,depart_level,one_sixth,Data_out, &
!$OMP&        dim_k_out,dim_j_out,dim_i_out,Ext_Data,i_out,j_out)
      Do k = 1, dim_k_out
...
! End of routine.
      IF (lhook) CALL dr_hook('ECMWF_QUASI_CUBIC_NIV',zhook_out,zhook_handle)
      RETURN
