...
! Local Variables.
     ...
     &, coeff_i_minus_l(dim_i_out, dim_j_out)                           &
     &, coeff_i_zero_l(dim_i_out, dim_j_out)                            &
     &, coeff_i_plus_l(dim_i_out, dim_j_out)                            & 
     &, coeff_i_plus2_l(dim_i_out, dim_j_out)                           &
...
! ----------------------------------------------------------------------
      ...
      IF (lhook) CALL dr_hook('ECMWF_QUASI_CUBIC',zhook_in,zhook_handle)
...
!$OMP& coeff_j_plus,coeff_j_zero,coeff_i_plus2_l,coeff_i_plus_l,        &
!$OMP& coeff_j_minus,coeff_i_plus2,coeff_i_plus,coeff_i_zero,           &
!$OMP& coeff_i_minus, coeff_j_plus2,coeff_j_lin1,coeff_i_zero_l,        & 
!$OMP& coeff_i_minus_l) SHARED(weight_phi,dim_k_out,dim_j_out,dim_i_out,&
...
!       Do j = 1, dim_j_out
        ...
        Do i = 1, dim_i_out*dim_j_out
            ...
            coeff_j_plus(i,j)  = 0.5 * (coeff_j_plus2(i,j) - phi_sq     &
     &                           - weight_phi(i,j,k))
            coeff_j_zero(i,j)  = 0.5 * (coeff_j_plus2(i,j) - 2. *       &
     &                                     phi_sq + 2.)
     ...
     &                                    2. * weight_phi(i,j,k))
            ...
            coeff_i_plus(i,j)  = 0.5 * (coeff_i_plus2(i,j)              &
     ...
     &                                      2. * weight_lambda(i,j,k))
            ...
            coeff_i_plus2_l(i,j) = (1.-weight_lambda(i,j,k)) *          &
     &                           (1.-weight_phi(i,j,k))
            coeff_i_plus_l(i,j)  = weight_lambda(i,j,k) *               &
     &                           (1.-weight_phi(i,j,k))
            coeff_i_zero_l(i,j)  = (1.-weight_lambda(i,j,k)) *          &
     &                            weight_phi(i,j,k)
            coeff_i_minus_l(i,j) = weight_lambda(i,j,k) *               &
     &                           weight_phi(i,j,k)
        End Do !i
!       End Do !j
...
! interpolate in i and store.
                ...
                col_data_i_j_n(index)  = coeff_i_plus2(i,j)             &
     &                                   * val_i_plus2                  &
     &                                 - coeff_i_plus(i,j)              &
     &                                   * val_i_plus                   &
     &                                 + coeff_i_zero(i,j)              &
...
! interpolate using bi-linear.
            Do index = -1, 2, 3
              ...
              col_data_i_j_n (index) = coeff_i_plus2_l(i,j) *           &
...
! Interpolate data
            Data_out (i,j,k,n) = coeff_z(i,j,k,-1) * col_data_i_j_n(-1) &
...
! End of routine.
      IF (lhook) CALL dr_hook('ECMWF_QUASI_CUBIC',zhook_out,zhook_handle)
      RETURN
