...
IF (lhook) CALL dr_hook('EG_DX_OUT_GRID',zhook_in,zhook_handle)
...
SELECT CASE(fld_type)
   ...
   CASE(4)
      cgrd = 'p_grid'//cpe//'.dat'
   CASE DEFAULT
       CALL Ereport('eg_dx_out_grid',1,'Unrecognized field type')
END SELECT
...
IF (lhook) CALL dr_hook('EG_DX_OUT_GRID',zhook_out,zhook_handle)
RETURN
