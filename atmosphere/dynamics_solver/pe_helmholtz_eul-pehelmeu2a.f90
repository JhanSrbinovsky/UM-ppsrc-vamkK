...
! 3-d work arrays
     ...
     &,etadot_star2
      ...
      Real, Allocatable, Target :: v_star_minus_v(:,:,:)
...
! ----------------------------------------------------------------------
      ...
      IF (lhook) CALL dr_hook('PE_HELMHOLTZ_EUL',zhook_in,zhook_handle)
...
! levels
!$OMP PARALLEL DEFAULT(NONE) PRIVATE(i,j,k,temp1)                       &
!$OMP& SHARED(model_levels,j0,j1,row_length,weight_upper,weight_lower,  &
!$OMP&        r_rho_levels,r_theta_levels,wet_model_levels,weight1,q,   &
!$OMP&        moist,q_star,moist_star,theta,theta_star,dry_density,     &
!$OMP&        HM_Cxx2,thetav_star)
...
!$OMP PARALLEL DO SCHEDULE(STATIC) DEFAULT(NONE) PRIVATE(i,j,k,interp3) &
!$OMP& SHARED(model_levels,j0,j1,row_length,theta,weight_upper,HM_RHS,  &
!$OMP&       thetav_star,thetav_star_rho,weight_lower,HM_Cyy2,HM_Cxy2,  &
!$OMP&        wet_model_levels,q,q_star,L_qwaterload,HM_Cxx2,           &
...
!$OMP&        dtheta_dr_term,dry_density)
      Do k = 1, model_levels - 1
        Do j = 1, rows
          Do i = 1, row_length
...
! end loop over levels
      ...
      IF (lhook) CALL dr_hook('PE_HELMHOLTZ_EUL',zhook_out,zhook_handle)
      RETURN
