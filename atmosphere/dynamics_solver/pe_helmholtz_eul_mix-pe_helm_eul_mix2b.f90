...
! *****************************COPYRIGHT*******************************
!
! Subroutine PE_Helmholtz_eul_mix_2B
      SUBROUTINE PE_Helmholtz_eul_mix_2B(                               &
...
! ----------------------------------------------------------------------
      ...
      IF (lhook) CALL dr_hook('PE_HELMHOLTZ_EUL_MIX_2B',zhook_in,zhook_handle)
     ...
     &                                       solver_row_length
      ...
      if( model_domain == mt_LAM ) then
        ...
        if (at_extremity(PNorth)) then
          ...
          j_end = j_stop
          j_stop_v = j_stop - 1
          if (L_transparent) j_stop_v = j_stop_v + 1
        endif ! at_extremity(PNorth)
        ...
        if (at_extremity(PEast)) then
          ...
          i_stop_u = i_stop - 1
          if (L_transparent) i_stop_u = i_stop_u + 1
...
! levels
!$OMP  PARALLEL DEFAULT(SHARED) PRIVATE(i, j, k, convert_to_v, temp3,      &
!$OMP& recip_Cp, u_star, interp2, interp3, interp1, weight1, weight2, ju,&
!$OMP& jv, temp4, jj, temp1, temp2, omp_block)                

     !$ omp_block = CEILING(( (j_stop+1) - (j_start-1) + 1)/ REAL(omp_get_num_threads()))
        ...
        DO k = 1, wet_model_levels
          DO j = 1-off_y, rows+off_y
            DO i = 1-off_x, row_length+off_x
              moist_np1(i,j,k) = mix_v_np1(i,j,k) + mix_cl_np1(i,j,k) + mix_cf_np1(i,j,k)
            END DO
          END DO
        END DO  
!$OMP END DO
...
!$OMP DO SCHEDULE(STATIC)
          DO k = 1, wet_model_levels
            DO j = 1-off_y, rows+off_y
              DO i = 1-off_x, row_length+off_x
                moist_np1(i,j,k) = moist_np1(i,j,k) + mix_cf2_np1(i,j,k)
              END DO
            END DO
          END DO  
!$OMP END DO
...
!$OMP DO SCHEDULE(STATIC)
          DO k = 1, wet_model_levels
            DO j = 1-off_y, rows+off_y
              DO i = 1-off_x, row_length+off_x
                moist_np1(i,j,k) = moist_np1(i,j,k) + mix_rain_np1(i,j,k)
              END DO
            END DO
          END DO 
!$OMP END DO        
...
!$OMP DO SCHEDULE(STATIC)
          DO k = 1, wet_model_levels
            DO j = 1-off_y, rows+off_y
              DO i = 1-off_x, row_length+off_x     
                moist_np1(i,j,k) = moist_np1(i,j,k) + mix_graup_np1(i,j,k)
              END DO
            END DO
          END DO 
!$OMP END DO
...
!$OMP DO SCHEDULE(STATIC)
      Do k = 1, model_levels - 1
        Do j = 1, rows
          Do i = 1, row_length
            interp2(i,j) = 1.0 - temp1 * ( exner_rho_levels(i,j,k+1) -  &
     &                                      exner_rho_levels(i,j,k)) /  &
...
! since u' is known at boundary and is held in R_u
     ...
     &                                      alpha_1 * R_u(i,j_begin-1,k)
      ...
      &                                    alpha_1 * R_u(i_start-1,j,k)
          ...
          If(at_extremity(PEast)) Then
            Do j = j_start, j_stop
              u_star(i_stop+1,j) = u(i_stop+1,j,k) +                    &
     ...
     &                             wt_lambda_u(i) * R_u(i-1,j,k)
        ...
        Do j = j_start-1, j_stop
          Do i = i_start, i_stop
            HM_Cz(i,j,k) = v(i,j,k) + alpha_1 *                         &
     &                       ( Av(i,j,k) * R_v(i,j,k) - Fv(i,j,k) *     &
     &                           ( wt_phi_v(i,j+1) * interp1(i,j) +     &
     &                     (1.0 - wt_phi_v(i,j+1)) * interp1(i,j+1) ) )
          End Do
        End Do
...
!$OMP  PARALLEL DEFAULT(SHARED) PRIVATE(i, j, k, jj, temp2, temp1,      &
!$OMP& weight1,  weight2, omp_block)

      !$ omp_block = CEILING((j_stop - j_start + 1)/ REAL(omp_get_num_threads()))
...
! calculate d(eta)/d(r) at theta levels
       ...
       &                       / temp2
            ...
            Do j = jj, MIN(jj+omp_block-1, j_stop)
              Do i = i_start, i_stop
                interp4(i,j) = etadot_star2(i,j,k) *                    &
       &          dtheta_dr_term(i,j,k) * convert_to_vstar(i,j,k)
...
! calculate d(eta)/d(r) at theta levels
       ...
       &                       / temp2
            ...
            Do j = jj, MIN(jj+omp_block-1, j_stop)
              Do i = i_start, i_stop
                interp4(i,j) = interp3(i,j)
                interp3(i,j) = etadot_star2(i,j,k) *                    &
       &          dtheta_dr_term(i,j,k) * convert_to_vstar(i,j,k)
                temp1 = interp5(i,j) * K_term(i,j,k)
                HM_Czz(i,j,k) = HM_C5(i,j,k) * temp1
                HM_Cz(i,j,k) = temp1 * dtheta_dr_term(i,j,k) *          &
...
! DEPENDS ON: array_l2norm
        ...
        WRITE(6,*)'Level 1 to ', model_levels,                          &
...
! since these are used to set the initial residuals.
        ...
        Else !  Original lbcs  or fixed lbcs
...
! DEPENDS ON: helm_l2_print
        CALL helm_l2_print(                                             &
                           ...
                           row_length, rows, model_levels,              &
                           first_constant_r_rho_level_m1,               &
                           k, k, off_x, off_y, .false., .false.,        &
                           rims_to_do, L_print_allpe, me )
        END DO ! k = 1, model_levels
...
! DEPENDS ON: helm_l2_print
        CALL helm_l2_print(                                             &
                           ...
                           row_length, rows, model_levels,              &
                           first_constant_r_rho_level_m1,               &
                           norm_start_lev, norm_end_lev,                &
                           off_x, off_y, .false., .false.,              &
                           rims_to_do, L_print_allpe, me )
...
!$OMP PARALLEL DEFAULT(SHARED) PRIVATE(interp6, interp5, interp2, i, j, &
!$OMP& interp1, temp1, k)
...
!$OMP END PARALLEL 
      ...
      IF (lhook) CALL dr_hook('PE_HELMHOLTZ_EUL_MIX_2B',zhook_out,zhook_handle)
      RETURN

      END SUBROUTINE PE_Helmholtz_eul_mix_2B
