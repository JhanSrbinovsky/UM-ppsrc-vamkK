...
!----------------------------------------------------------------
! Description:
!  Given a netcdf file and variable name return 4D variable
...
!------------------------------------------------------------------
SUBROUTINE nudging_netcdf_vareader_4d(   &
 filename,                               &  ! Filename
...
INTEGER, INTENT(IN) :: dim4          ! dimension 4 length

REAL, INTENT(OUT)   :: variable(dim1, dim2, dim3, dim4)
...
INTEGER(KIND=integer32) :: numdim       ! no. dimensions
INTEGER(KIND=integer32) :: mydimids (4) ! array of dimension IDs
INTEGER(KIND=integer32) :: dimlen (4)   ! read dimension lengths
INTEGER                 :: dims (4)     ! expected dimension lengths
...
IF (lhook) CALL dr_hook('NUDGING_NETCDF_VAREADER_4D',zhook_in,zhook_handle)
...
IF(debug.gt.10) THEN
  WRITE(OUT,*)                                                         &
   ': NUDGING_NETCDF_VAREADER (4D): Entered routine'
END IF
...
! Check no.dimensions is correct. If not return error
 IF(numdim.ne.4) THEN
   nmessage = 'Error Number dimenions NE 4' //varname
   status = 999
...
! oop  over dimensions check input and netcdf dimensions same
DO i=1, numdim
...
IF(debug.gt.10) WRITE(OUT,*)                                            &
  ': NUDGING_NETCDF_VAREADER (4D): about to fill variable ',           &
...
IF(debug.gt.10) THEN
  WRITE(OUT,*)                                                         &
  ': NUDGING_NETCDF_VAREADER (4D): Leaving routine'
END IF

IF (lhook) CALL dr_hook('NUDGING_NETCDF_VAREADER_4D',zhook_out,zhook_handle)

RETURN
END SUBROUTINE nudging_netcdf_vareader_4d
