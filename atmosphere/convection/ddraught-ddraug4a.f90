...
!
SUBROUTINE ddraught_4a (npnts, np_full, k, kct, ntra,                        &
                     l_tracer,                                               &
                     thdd_k,qdd_k,the_k,                                     &
...
IF (lhook) CALL dr_hook('DDRAUGHT_4A',zhook_in,zhook_handle)
...
DO i=1,npnts
  flx_dd_km1(i) = flx_dd_k(i)*(1.0+ekm34(i))*(1.0+ekm14(i))*(1.0-amdetk(i))
...
CALL satcal_4a(npnts,k,tdd_km1,thdd_km1,p_km1,exkm1,qdd_km1,the_km1,qse_km1,  &
               qdds,thdds) 
...
IF (lhook) CALL dr_hook('DDRAUGHT_4A',zhook_out,zhook_handle)
RETURN

END SUBROUTINE ddraught_4a
