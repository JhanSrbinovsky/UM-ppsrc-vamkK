...
IF (lhook) CALL dr_hook('MID_CONV_5A',zhook_in,zhook_handle)
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
   ...
   IF (.NOT. l_eman_dd) THEN       ! original down draughts
...
DO i=1,npnts
  IF (kterm_mid(i) == nlev-1 .AND. l_mid_all(i) ) THEN
    IF (printstatus >= prstatus_normal) THEN
      ...
      ! Not formatted as not appropriate, nlev can vary.
      write(6,*) ' theta ',(th(i,k),k=1,nlev)
      write(6,*) ' q ',(q(i,k),k=1,nlev)
      write(6,*) ' qcl ',(qcl(i,k),k=1,nlev)
      write(6,*) ' qcf ',(qcf(i,k),k=1,nlev)
      write(6,*) ' up mass ',(flx(i,k),k=1,nlev)
      write(6,*) ' dq ',(dqbydt(i,k),k=1,nlev)
      write(6,*) ' dqcl ',(dqclbydt(i,k),k=1,nlev)
      write(6,*) ' dqcf ',(dqcfbydt(i,k),k=1,nlev)
      write(6,*) ' dth ',(dthbydt(i,k),k=1,nlev)
      write(6,*) ' du ',(dubydt(i,k),k=1,nlev)
      write(6,*) ' dv ',(dvbydt(i,k),k=1,nlev)
    END IF
    error_point = i   
    IF (lhook) CALL dr_hook('MID_CONV_5A',zhook_out,zhook_handle)
    RETURN        
  END IF
    ...
    k=1
      DO j = 1,nmid
        ...
        IF (temp1(j)  <   0.0 .AND.                              &
          ...
          WRITE(6,'(a19,i6,a9,g26.18,a7,g26.18)') ' negative q mid, i:',     &
