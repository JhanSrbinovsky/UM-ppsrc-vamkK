...
!
SUBROUTINE shallow_conv_4a(nbl,nlev,ntra,n_cca_lev,n_sh,trlev,    &
...
USE cv_run_mod, ONLY:                                             &
    l_mom, sh_pert_opt, limit_pert_opt, bl_cnv_mix,               &
    cca2d_sh_opt, cca_sh_knob, ccw_sh_knob, l_dcpl_cld4pc2,       &
    cnv_wat_load_opt, l_ccrad

USE cv_param_mod, ONLY:                                           &
    total_condensed_water, grant_lock,                            &
    thpixs_shallow, qpixs_shallow, c_mass,                        &
    max_sh_thpert, min_sh_thpert, max_sh_qpert_fac
...
REAL, INTENT(IN)    :: p_layer_centres(n_sh,0:nlev) ! Pressure (Pa)
...
REAL, INTENT(IN)    :: &
 ...
 ,zlcl_uv(n_sh)        & ! Lifting condensation level defined for the uv grid(m)
...
REAL, INTENT(INOUT) :: bulk_cf(n_sh,nlev) ! Bulk total cloud volume ( )

REAL, INTENT(INOUT) :: cf_frozen(n_sh,nlev) ! Frozen water cloud volume ( )

REAL, INTENT(INOUT) :: cf_liquid(n_sh,nlev) ! Liq water cloud volume ( )

REAL, INTENT(INOUT) :: qcf(n_sh,nlev) ! Ice condensate mix ratio (kg/kg)

REAL, INTENT(INOUT) :: qcl(n_sh,nlev) ! Liq condensate mix ratio (kg/kg)

REAL, INTENT(INOUT)    :: tracer(n_sh,trlev,ntra) !Model tracer fields (kg/kg)
...
REAL, INTENT(OUT) :: cclwp(n_sh)! Condensed water path (kg/m^2)

REAL, INTENT(OUT) :: ccw(n_sh,nlev)
                                ! Convective cloud liquid water
                                ! on model levels (kg/kg)

REAL, INTENT(OUT) :: cca(n_sh,n_cca_lev)
                                ! Convective cloud amount on model
                                ! levels (0-1)
...
INTEGER, INTENT(OUT) :: iccb(n_sh) ! Convective cloud base level

INTEGER, INTENT(OUT) :: icct(n_sh) ! Convective cloud top level

REAL, INTENT(OUT) :: lcca(n_sh) ! Lowest conv. cloud amt. (%)

INTEGER, INTENT(OUT) :: lcbase(n_sh) ! Lowest conv. cloud base level

INTEGER, INTENT(OUT) :: lctop(n_sh) ! Lowest conv. cloud top level
...
REAL :: flxkp12(nlev,n_sh)      ! Mass flux on half level (Pa/s)
...
REAL :: thpk_c(n_sh), thpk_c2(n_sh) ! Parcel potential temp in layer k (K)

REAL :: thpk(n_sh)              !ad. entrain.

REAL :: thpkp1_c(n_sh), thpkp1_c2(n_sh)! Parcel potential temp in layer k (K)
...
REAL :: upk_c(n_sh), upk_c2(n_sh) ! Parcel U in layer k after entrainment (m/s)

REAL :: upkp1_c(n_sh), upkp1_c2(n_sh) ! Parcel U in layer k+1
                                ! after entrainment (m/s)

REAL :: vpk_c(n_sh), vpk_c2(n_sh) ! Parcel V in layer k after entrainment (m/s)
...
REAL :: overlap_fac(n_sh) ! Factor designed to improve shallow Cu
                          ! cover by allowing for non-vertical clouds
...
IF (lhook) CALL dr_hook('SHALLOW_CONV_4A',zhook_in,zhook_handle)
...
DO k = 1,nlev-1
  DO i = 1,n_sh
    ...
    thpixs_v(i,k) = MIN( ((p_layer_centres(i,k) -               &
              p_layer_centres(i,k+1))/5000.),1.0) * thpixs_shallow
...
IF (sh_on == 1) THEN   ! Adaptive forced detrainment on
...
! DEPENDS ON: layer_cn_4a
  CALL layer_cn_4a(k,n_sh,n_sh,nlev,                                 &
...
! DEPENDS ON: dqs_dth_4a
    CALL dqs_dth_4a(dqsthk,k,th(1,k),qse(1,k),exk,n_sh)
  ELSE
    DO i = 1,n_sh
      dqsthk(i) = dqsthkp1(i)
    END DO
...
! DEPENDS ON: dqs_dth_4a
  CALL dqs_dth_4a(dqsthkp1,k+1,th(1,k+1),qse(1,k+1),exkp1,n_sh)
...
! DEPENDS ON: lift_par_4a
  CALL lift_par_4a(ncposs,n_sh,thpkp1_c,qpkp1_c,xsqkp1_c,         &
...
! Note CAPE not applied to shallow convection
...
! DEPENDS ON: mix_ipert_4a5a
  CALL mix_ipert_4a5a(n_sh, n_sh, nlev, nbl, ntml, index1,            &
                      p_layer_boundaries,                             &
...
IF (l_ccrad) THEN
  ...
  SELECT CASE (cca2d_sh_opt)
    ...
    CASE(grant_lock)
      DO i=1, n_sh
        IF (iccb(i) /= 0) THEN ! Shallow convection occurred

          tempnum = 2.0*mb(i) / (  (delthvu(i)*mb(i)*g                     &
                      / ( th(i,ntml(i))*(1.0+0.61*q(i,ntml(i)))))**0.3333)
  ...
  IF (.NOT. l_dcpl_cld4pc2) THEN
      ...
      !---------------------------------------------------------------
      ! Because of decoupling move up a level so scaling is only
      ! applied to section 0 variables, section 5 variables continue
      ! to be scaled here to maintain bit-comparison, would ideally
      ! be moved up to Glue_Conv in future
  ...
  !-------------------------------------------------------------------
  ! 7.2 Fill cca with cca_2d where non-zero ccw
  !-------------------------------------------------------------------
  DO k=1, nlev
    DO i=1, n_sh
      IF (iccb(i) /= 0) THEN ! Shallow convection occurred
...
IF (lhook) CALL dr_hook('SHALLOW_CONV_4A',zhook_out,zhook_handle)
RETURN
END SUBROUTINE shallow_conv_4a
