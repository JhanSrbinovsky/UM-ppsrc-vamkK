...
USE cv_run_mod, ONLY:                                                   &
    iconv_congestus, icvdiag, cape_opt, w_cape_limit, iconv_deep,       &
...
REAL :: column_q_bl(nunstable)        ! Column integrated q in BL
REAL :: qsat_c(nunstable, wet_levels) ! Qsat
...
CHARACTER (LEN=17), PARAMETER ::  routinename = 'conv_diag_comp_6a'
...
IF (lhook) CALL dr_hook('CONV_DIAG_COMP_6A',zhook_in,zhook_handle)
...
ELSE IF (icvdiag >= 2 .AND. icvdiag < 15 ) THEN
  ...
  !-----------------------------------------------------------------------------
  ! TEST options - icvdiag == 6 and above removed
  !-----------------------------------------------------------------------------

  ELSE IF (icvdiag == 10 ) THEN ! Ben's diagnosis
    
      CALL calc_column_rh(nunstable, wet_levels, nlcl_c         &
         , row_length,rows,model_levels                         &
         , index_i, index_j                                     &
         , p_c, T, q_c, rho_only                                &
         , z_half_c, l_mixing_ratio, 20000.                     &
...
!-----------------------------------------------------------------------
        IF ( shmin(ii) .AND. zcld < zcld_thresh ) THEN
          ...
          IF ( (z_full_c(ii,kshmin(ii)) - z_full_c(ii,ntpar_l))          &
            ...
            ELSEIF (zcld <=  2000. .AND. zcld >  500.)THEN
...
IF (lhook) CALL dr_hook('CONV_DIAG_COMP_6A',zhook_out,zhook_handle)
RETURN
