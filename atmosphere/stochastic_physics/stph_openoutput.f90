...
SUBROUTINE stph_openoutput(io_pos, l_coefdump)
...
! Random seed will always be written out if l_stphseed_write is true
...
! Local variables
 ...
 IF (lhook) CALL dr_hook('STPH_OPENOUTPUT',zhook_in,zhook_handle)
...
! Output mode will be used in CRUNs after initial read
 IF (mype == 0) THEN
   IF (l_stphseed_write.OR.l_stphseed_read) THEN
     ...
     IF (icode == 0) THEN
       WRITE(6, *) 'Successfully retrieved RPSEED filename = ', filename
     ELSE
       ...
       IF (icode /= 0) WRITE(6,*) &
           'RPSEED OUT: Failed to get value of  $DATAW'

       CALL fort_get_env('RUNID',5,runidstring,runidnamelength,icode)
       IF (icode /= 0) WRITE(6,*)&
           'RPSEED OUT: Failed to get value of $RUNID'
     ...
     IF (LEN_TRIM(filename) == 0) THEN
       WRITE(6,*) '*** WARNING *** Length of filename is equal to zero'
       CALL fort_get_env('DATAW',5,datawstring,datawnamelength,icode)
       IF (icode /= 0) WRITE(6,*) &
           'RPSEED OUT: Failed to get value of $DATAW'
       WRITE(6,*) 'Retrieved DATAW=',datawstring

       CALL fort_get_env('RUNID',5,runidstring,5,icode)
       IF (icode /= 0) WRITE(6,*) &
           'RPSEED OUT: Failed to get value of $RUNID'
       WRITE(6,*) 'Retrieved RUNID=',runidstring

       filename = TRIM(datawstring) // "/" // TRIM(runidstring) //      &
                  & ".seed"
       WRITE(6, *) 'Assumed Filename=', filename
     END IF
     ...
     !----------------------------------------------------------------
     OPEN(UNIT=149, FILE=filename, ACTION='write',                      &
          POSITION=io_pos, FORM=fileformat, IOSTAT=icode)
     IF(icode > 0) WRITE(6,*) "RPSEED OUT: Failed to open ", filename,  &
                              " icode=",icode,"   io_pos=",io_pos
     IF(icode == 0) WRITE(6,*) "RPSEED OUT: ",filename, "opened ,form=",&
                               fileformat," at position=",io_pos

   END IF  ! L_STPHSEED_WRITE or L_STPHSEED_READ
 END IF

 IF (lhook) CALL dr_hook('STPH_OPENOUTPUT',zhook_out,zhook_handle)
 RETURN
