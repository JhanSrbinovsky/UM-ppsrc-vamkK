...
! -----------------------------------------------------------
      ...
      IF (lhook) CALL dr_hook('LBC_SRC_SETUP',zhook_in,zhook_handle)
      ErrorStatus=0
      CMessage = ' '
...
! Halo Type
        ...
        level_type = exppxi                                             &
     &  (im_ident, sect_prog(var), item_prog(var), ppx_lv_code,         &
     &   ErrorStatus, cmessage)

        first_level = exppxi                                            &
     &  (im_ident, sect_prog(var), item_prog(var), ppx_lb_code,         &
     &   ErrorStatus, cmessage)

        last_level = exppxi                                             &
     &  (im_ident, sect_prog(var), item_prog(var), ppx_lt_code,         &
     &   ErrorStatus, cmessage)

        grid_type  = exppxi                                             &
     &  (im_ident, sect_prog(var), item_prog(var), ppx_grid_type,       &
     &   ErrorStatus, cmessage)

        halo_type  = exppxi                                             &
     &  (im_ident, sect_prog(var), item_prog(var), ppx_halo_type,       &
     &   ErrorStatus, cmessage)
...
!----------------------------
      ...
      IF (lhook) CALL dr_hook('LBC_SRC_SETUP',zhook_out,zhook_handle)
      RETURN
