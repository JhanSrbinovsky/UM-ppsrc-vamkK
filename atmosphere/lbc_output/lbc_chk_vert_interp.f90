...
! Local variables
      ...
      IF (lhook) CALL dr_hook('LBC_CHK_VERT_INTERP',zhook_in,zhook_handle)
      Do jintf = 1, n_intf_a

! -------------------------------------
! Check vertical interpolation is valid
! -------------------------------------
        IF (intf_v_int_order(jintf) /= interp_order_linear         .AND. &
            intf_v_int_order(jintf) /= interp_order_linear_noex    .AND. &
            intf_v_int_order(jintf) /= interp_order_cubic          .AND. &
            intf_v_int_order(jintf) /= interp_order_quintic) THEN
          errorstatus = 1
          cmessage    = "Vertical interpolation method not recognised."
     ...
     &           lbc_eta_rho(jlev,jintf) ) > Epsilon (1.0) ) Then
        ...
        If (PrintStatus >= PrStatus_Normal ) Then
          If ( Intf_Vert_Interp(jintf) ) Then
            WRITE (6,'(A,A,I2)')                               &
              'LBC_CHK_VI : Vertical Interpolation required ', &
      ...
      IF (lhook) CALL dr_hook('LBC_CHK_VERT_INTERP',zhook_out,zhook_handle)
      RETURN
