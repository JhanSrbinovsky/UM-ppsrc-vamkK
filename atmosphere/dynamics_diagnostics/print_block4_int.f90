...
! ----------------------------------------------------------------------
      ...
      IF (lhook) CALL dr_hook('PRINT_BLOCK4_INT',zhook_in,zhook_handle)
...
! Length = size = max_x*max_y*num_lev arrays
        ...
        CALL gc_isum(size, n_proc, info, ll)
        CALL gc_isum(size, n_proc, info, lr)
        CALL gc_isum(size, n_proc, info, ul)
        CALL gc_isum(size, n_proc, info, ur)
...
! ----------------------------------------------------------------------
        ...
        DO k = 1, num_lev
          ...
          ii = dom_w
          IF (ix == 4) THEN
            WRITE(6,'(''column          '',I4,''        '', I4,         &
            ''        '',I4,''        '',I4)') ii, ii+1, ii+2, ii+3
          ELSE IF (ix == 3) THEN
            WRITE(6,'(''column          '',I4,''        '', I4,         &
            ''        '',I4)') ii, ii+1, ii+2
          ELSE IF (ix == 2) THEN
            WRITE(6,'(''column          '',I4,''        '', I4)')ii,ii+1
          ELSE
            WRITE(6,'(''column          '',I4)')ii
          END IF !  ix == 4
          DO j = 1, jy
            jn = dom_n - j + 1
            WRITE(6,'(''row '',I4,4I12)') jn, (ul(i,j,k), i = 1, ix)
          END DO
          ...
          DO j = 1, jy
            jn = dom_n - j + 1
            WRITE(6,'(''row '',I4,4I12)') jn, (ur(i,j,k), i = 1, ix)
          END DO
          ...
          DO j = 1, jy
            jn = dom_s + jy - j
            WRITE(6,'(''row '',I4,4I12)') jn, (ll(i,j,k), i = 1, ix)
          END DO
          ...
          DO j = 1, jy
            jn = dom_s + jy - j
            WRITE(6,'(''row '',I4,4I12)') jn, (lr(i,j,k), i = 1, ix)
          END DO
      ...
      IF (lhook) CALL dr_hook('PRINT_BLOCK4_INT',zhook_out,zhook_handle)
      RETURN
