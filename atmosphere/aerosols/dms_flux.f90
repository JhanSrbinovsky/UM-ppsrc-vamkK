...
SUBROUTINE dms_flux(                                               &
  ! Arguments IN
  row_length, rows,                                                &
  u_1, v_1, height, tstar, land_fract, dms_conc,                   &
...
REAL    :: u_1(row_length, rows)        ! Level 1 u-wind (ms-1)
REAL    :: v_1(row_length, rows)        ! Level 1 v-wind (ms-1)
REAL    :: height(row_length, rows)     ! Level 1 centre height (m)
...
IF (lhook) CALL dr_hook('DMS_FLUX',zhook_in,zhook_handle)
...
IF (lhook) CALL dr_hook('DMS_FLUX',zhook_out,zhook_handle)
RETURN
END SUBROUTINE dms_flux
END MODULE dms_flux_mod
