...
REAL, INTENT(OUT) :: hno3_mmr_out(row_length,rows,model_levels)
...
! Local arrays:
!
REAL, DIMENSION(:,:,:), ALLOCATABLE :: o3_mmr   ! O3 mass mixing ratio (kg[O3]/kg[air])
REAL, DIMENSION(:,:,:), ALLOCATABLE :: hno3_mmr ! HNO3 mass mixing ratio (kg[HNO3]/kg[air])
REAL, DIMENSION(:,:,:), ALLOCATABLE :: h2o2_mmr ! H2O2 mass mixing ratio (kg[H2O2]/kg[air])
REAL, DIMENSION(:,:,:), ALLOCATABLE :: oh_mmr   ! OH mass mixing ratio (kg[OH]/kg[air])
REAL, DIMENSION(:,:,:), ALLOCATABLE :: ho2_mmr  ! HO2 mass mixing ratio (kg[HO2]/kg[air])
REAL, DIMENSION(:,:,:), ALLOCATABLE :: oh_conc  ! OH concentration (molecules/cm3)
REAL, DIMENSION(:,:,:), ALLOCATABLE :: ho2_conc ! HO2 concentration (molecules/cm3)
...
REAL, DIMENSION(:,:,:), ALLOCATABLE :: air_density    ! molecules / cm3

CHARACTER(LEN=80) :: cmessage         ! Error message

! Denotes whether or not each UKCA oxidant has been found in D1 array
...
IF (l_sulpc_online_oxidants) THEN
  ...
  array_size = SIZE(hno3_mmr)
  hno3_mmr = RESHAPE(hno3_ukca,                                                &
    (/ row_length+(2*x_halo_hno3), rows+(2*y_halo_hno3),                       &
    model_levels /))
  ...
  DO k=1,model_levels
    DO j=1,rows
      DO i=1,row_length
        ...
        air_density(i,j,k) = p_theta_levels_local(i,j,k) *                     &
          avogadro * 1.0e-06 / (rmol * t(i,j,k))
...
ELSE
  ...
  ! oxidants from D1 array (OH, HO2, O3 and H2O2), or set to zero (HNO3).
  ALLOCATE(o3_mmr(row_length, rows, model_levels))
  ALLOCATE(hno3_mmr(row_length, rows, model_levels))
...
hno3_mmr_out(:,:,:) = hno3_mmr(1:row_length,1:rows,1:model_levels)
...
IF (lhook) CALL dr_hook('GET_SULPC_OXIDANTS',zhook_out,zhook_handle)
RETURN
