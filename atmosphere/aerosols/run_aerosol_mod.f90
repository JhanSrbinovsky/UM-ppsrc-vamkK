...
! *****************************COPYRIGHT*******************************
!
!   A module containing tunable parameters used in the aerosol scheme
!
...
l_so2,        l_so2_hilem, l_soot,           &                
l_soot_hilem, l_bmass_hilem,                 &
l_ocff,       l_ocff_hilem                  
...
! 
  NAMELIST/RUN_Aerosol/                                             &
    ...
    BMass_high_level_2, ocff_high_level
...
!
  ...
  INTEGER icode
!
!   Check that a user-defined emissions heights are sensible
!
  icode = 0
  IF (L_SO2_hilem) THEN
    IF (SO2_high_level == imdi) THEN
      IF (mype == 0) &
      WRITE(6,'(A)') 'Error - RUN_aerosol - SO2 emission but level not set'
      icode = icode + 1
    ELSE IF (SO2_high_level < 1 .OR. SO2_high_level > model_levels) THEN
      IF (mype == 0) &
      WRITE(6,'(A,2I6)') 'Error - RUN_aerosol - Query SO2_high_level',&
...
!
  IF (L_Soot_hilem) THEN
    IF (Soot_high_level == imdi) THEN
      IF (mype == 0) &
      WRITE(6,'(A)') 'Error - RUN_aerosol - Soot emission but level not set'
      icode = icode + 1
    ELSE IF (Soot_high_level < 1 .OR. Soot_high_level > model_levels) THEN
      IF (mype == 0) &
      WRITE(6,'(A,2I6)') 'Error - RUN_aerosol - Query Soot_high_level',&
                          Soot_high_level,model_levels
      icode = icode + 1
    END IF
  END IF
!
  IF (L_BMass_hilem) THEN
    IF (BMass_high_level_1 == imdi) THEN
      IF (mype == 0) &
      WRITE(6,'(A)') 'Error - RUN_aerosol - BMass emission but level1 not set'
      icode = icode + 1
    ELSE IF (BMass_high_level_1<1 .OR. BMass_high_level_1>model_levels) THEN
      IF (mype == 0) &
      WRITE(6,'(A,A,2I6)') 'Error - RUN_aerosol - ', &
...
!
  IF (L_BMass_hilem) THEN
    IF (BMass_high_level_2 == imdi) THEN
      IF (mype == 0) &
      WRITE(6,'(A)') 'Error - RUN_aerosol - BMass emission but level2 not set'
      icode = icode + 1
...
!
  IF (L_ocff_hilem) THEN
    IF (ocff_high_level == imdi) THEN
      IF (mype == 0) &
      WRITE(6,'(A)') 'Error - RUN_aerosol - ocff emission but level not set'
      icode = icode + 1
    ELSE IF (ocff_high_level < 1 .OR. ocff_high_level > model_levels) THEN
      IF (mype == 0) &
      WRITE(6,'(A,2I6)') 'Error - RUN_aerosol - Query ocff_high_level',&
  ...
  IF (icode /= 0) THEN
    cmessage = '*** Error(s) in level(s) being set in RUN_Aerosol namelist'
