...
REAL, PARAMETER :: P1 = ALOG(10.0/Z0_SEA)
...
IF (lhook) CALL dr_hook('AERO_CTL_4A',zhook_in,zhook_handle)
...
! calculate level 1 winds on p points using function
  CALL uv_p_pnts(u(1:row_length,1:rows,1),v(1:row_length,1:n_rows,1), &
    ...
    model_domain,global_row_length,proc_row_group,u_1,v_1)
!$OMP END MASTER
!$OMP BARRIER
...
! to handle ocean DMS flux at poles
! Include south pole in case we are running an Aquaplanet model
!$OMP MASTER
...
! Sum over points on PEs in order along first interior row:
      ...
      dms_ointer_mean(1)=dms_ointer_mean(1)/global_row_length
      DO i = 1, row_length
        dms_ointer(i, 1) = dms_ointer_mean(1)
      END DO
...
!
ELSE
! no special conditions since p-point not on pole in V-AT-POLES
END IF ! vatpoles
...
IF (l_biomass) THEN  ! If biomass smoke modelling is included
  ...
  IF (l_bmass_hilem) THEN  ! If high level smoke emissions
    ...
    IF (bmass_high_level_1  >   bmass_high_level_2 .OR.                        &
      ...
      WRITE(6,*) 'Aero_Ctl_4A: Invalid range of biomass emission '
...
! ARGSTS end
  stashwork17)
IF (lhook) CALL dr_hook('AERO_CTL_4A',zhook_out,zhook_handle)
RETURN
