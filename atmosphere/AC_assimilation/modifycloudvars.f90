...
!  Called by TDF not IAU

MODULE modifycloudvars_mod

IMPLICIT NONE

CONTAINS

SUBROUTINE ModifyCloudVars ( CC_tol,                 & ! in
                             qCL,                    & ! inout
                             qCF,                    & ! inout
...
!
!   1. Remove qCLs and qCFs below the given cloud clearing tolerance.
...
REAL, INTENT(IN)    :: CC_tol ! qCL/qCF tolerance for cloud clearing
...
REAL, INTENT(INOUT) :: bulk_cloud_fraction                     &
                           ( qdims_l%i_start : qdims_l%i_end,  &
                             qdims_l%j_start : qdims_l%j_end,  &
                                           1 : qdims_l%k_end )

REAL, INTENT(INOUT) :: cloud_fraction_liquid                   &
                           ( qdims_l%i_start : qdims_l%i_end,  &
                             qdims_l%j_start : qdims_l%j_end,  &
                                           1 : qdims_l%k_end )

REAL, INTENT(INOUT) :: cloud_fraction_frozen                   &
                           ( qdims_l%i_start : qdims_l%i_end,  &
                             qdims_l%j_start : qdims_l%j_end,  &
                                           1 : qdims_l%k_end )
...
IF (lhook) CALL dr_hook('MODIFYCLOUDVARS',zhook_in,zhook_handle)
...
DO k =             1, qdims%k_end
  DO j = qdims%j_start, qdims%j_end
    DO i = qdims%i_start, qdims%i_end
      ...
      IF (cloud_fraction_liquid(i,j,k) == 0.0 .AND. &
          cloud_fraction_frozen(i,j,k) == 0.0) THEN
...
IF (lhook) CALL dr_hook('MODIFYCLOUDVARS',zhook_out,zhook_handle)
RETURN
END SUBROUTINE ModifyCloudVars
END MODULE modifycloudvars_mod
