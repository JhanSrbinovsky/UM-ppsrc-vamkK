...
! this routine includes typptra.h, hence uses nearly every 
! variable type defined in the module below. Listing them all
! in the only statement would not be sinsible.
USE atm_fields_bounds_mod
USE eng_mass_diag_mod,     ONLY: eng_mass_diag
USE UM_ParVars,            ONLY: halo_i,halo_j,offx,offy
USE Control_Max_Sizes,     ONLY: max_req_thpv_levs,max_n_intf_a
...
! CTIME end
      ...
      CHARACTER(LEN=256) :: cmessage    ! Work - Internal error message
...
! Values in the dump will come from a previous climate integration.
      ...
      IF (lhook) CALL dr_hook('INIT_EMCORR',zhook_in,zhook_handle)
      IF (a_realhd(rh_tot_mass_init  ) == rmdi .OR.                     &
        ...
        CALL wet_to_dry_n_calc( D1(jq(qdims%k_start)),                  &
                                D1(jqcl(qdims%k_start)),                &
                                D1(jqcf(qdims%k_start)),                &
                                D1(jqcf2(qdims%k_start)),               &
                                D1(jqrain(qdims%k_start)),              &
                                D1(jqgraup(qdims%k_start)))
                  
        CALL eng_mass_diag (                                            &
                          wet_levels,                                   &
                          delta_lambda,delta_phi,                       &      
                          d1(jtheta(tdims%k_start)),                    &
                          d1(ju(udims%k_start)),                        &
                          d1(jv(vdims%k_start)),                        &
                          d1(jw(wdims_s%k_start)),                      &
                          d1(jrho(pdims%k_start)),                      &
                          d1(jq(qdims%k_start)),                        &
                          d1(jqcl(qdims%k_start)),                      &
                          d1(jqcf(qdims%k_start)),                      &
                          wet_to_dry_n,                                 & 
                          d1(jexner_theta_levels(tdims%k_start)),       &
...
! INITIAL RATE OF ENERGY CORRECTION TO ZERO
      ...
      IF (lhook) CALL dr_hook('INIT_EMCORR',zhook_out,zhook_handle)
      RETURN
