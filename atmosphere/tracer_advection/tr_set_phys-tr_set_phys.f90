...
!      local variables
      ...
      IF (lhook) CALL dr_hook('TR_SET_PHYS',zhook_in,zhook_handle)
      array_size_count=0
...
! DEPENDS ON: set_external_halos
          CALL set_external_halos(super_tracer_phys, row_length, rows,&
              array_size_count*model_levels, supertrdims%halo_i,      &
...
! DEPENDS ON: fill_external_halos
            CALL fill_external_halos(                                &
             super_tracer_phys(supertrdims%i_start,                  &
                            supertrdims%j_start,1, array_size_count) &
                         , row_length, rows,                         &
                           model_levels, supertrdims%halo_i,         &
                           supertrdims%halo_j)
          END IF
        END IF  ! L_Murk_advect
...
! DEPENDS ON: set_external_halos
           CALL set_external_halos(tracer_phys1, row_length,          &
              rows, tr_vars*tr_levels, supertrdims%halo_i,            &
              supertrdims%halo_j, 0.0)
          END IF
        END IF !L_init
      END IF  ! tr_vars > 0
...
! DEPENDS ON: set_external_halos
      ...
      IF (lhook) CALL dr_hook('TR_SET_PHYS',zhook_out,zhook_handle)
      RETURN
