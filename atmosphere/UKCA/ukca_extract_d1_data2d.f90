...
! TYPD1 end
        ...
        IF (lhook) CALL dr_hook('UKCA_EXTRACT_D1_DATA2D',zhook_in,zhook_handle)
        ...
        IF(first .AND. SIZE(X) /= UkcaD1Codes(N)%length) THEN           
          ...
          WRITE(6,*) 'Error: UKCAD1CODES(',N,')=', UkcaD1Codes(N)
        ...
        IF (lhook) CALL dr_hook('UKCA_EXTRACT_D1_DATA2D',zhook_out,zhook_handle)
        RETURN
