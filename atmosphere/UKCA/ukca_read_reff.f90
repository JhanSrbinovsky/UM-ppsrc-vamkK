...
! main program body
      IF (lhook) CALL dr_hook('UKCA_READ_REFF',zhook_in,zhook_handle)
...
! read and keep climatology
          OPEN(ukcafjar_unit,FILE=TRIM(ADJUSTL(dir_reff_sulp))//         &
      ...
      DO i=1,rows
        DO j=1,row_length
          ...
          ELSE
            ...
            aerosol_reff(j,i,:) = (1. - frac) * sad_clim1(index    ,:)+    &
                                      frac  * sad_clim1(index + 1,:)
          END IF
        END DO
      END DO

      WHERE (aerosol_reff < 0.) aerosol_reff = 0.

      IF (lhook) CALL dr_hook('UKCA_READ_REFF',zhook_out,zhook_handle)
