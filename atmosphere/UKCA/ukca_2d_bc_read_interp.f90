...
! Local variables
      ...
      LOGICAL             :: l_exist       ! Logical to test if files exist
      ...
      REAL                :: delpos                                                         
      CHARACTER (LEN=256) :: filename(3)   ! Full file name incl dir pathname 
      CHARACTER (LEN=256) :: filenm(3)     ! File name 
...
! **********************************************************************
!
      IF (lhook) CALL dr_hook('UKCA_2D_BC_READ_INTERP',zhook_in,zhook_handle)
...
!
      IF (mype == 0 .AND. firstcall) THEN                                             
        ...
        unit_nm(1) = ukca2do3_unit
        filenm(2) = '/o3_topbound.dat' 
        unit_nm(2) = ukca2ch4_unit
        filenm(3) = '/noy_topbound.dat' 
        unit_nm(3) = ukca2noy_unit
 
        DO ifile=1,3  
          filename(ifile) = TRIM(strat2d_dir)//filenm(ifile) 
          INQUIRE (FILE=filename(ifile), EXIST=l_exist) 
          IF (.NOT. l_exist) THEN 
            cmessage = filename(ifile)//': File does not exist' 
 
            CALL EREPORT('ukca_2d_bc_read_interp',ifile,cmessage) 
          END IF 
          OPEN(unit_nm(ifile),FILE=filename(ifile)) 
        END DO 
...
!     Interpolate in time                                             
      ...
      IF (lhook) CALL dr_hook('UKCA_2D_BC_READ_INTERP',zhook_out,zhook_handle)
