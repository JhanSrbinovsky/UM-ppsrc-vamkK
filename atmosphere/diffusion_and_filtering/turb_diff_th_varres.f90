...
! *****************************COPYRIGHT*******************************
!
! Subroutine turb_diff_th_varres
...
IF (lhook) CALL dr_hook('TURB_DIFF_TH_VARRES',zhook_in,zhook_handle)

!$OMP  PARALLEL DEFAULT(SHARED) PRIVATE(phi_term, i, j, k, temp,  &
!$OMP& lambda_term, delta_z, dt_over_r_squared_delz)
...
! ----------------------------------------------------------------------
  DO j = 1, rows
    DO i = 1-off_x, row_length
      temp(i,j) = (theta(i+1,j,k) - theta(i,j,k) )                &
                    * recip_dlamp(i)                              &
...
! Section 2.3  Calculate physics increment theta_star.
! ----------------------------------------------------------------------

  DO j = 0, rows
    DO i = 1, row_length
      temp(i,j) = (theta(i,j+1,k) - theta(i,j,k) )                &
                    * recip_dphip(i,j)                            &
...
IF (lhook) CALL dr_hook('TURB_DIFF_TH_VARRES',zhook_out,zhook_handle)
RETURN
END SUBROUTINE turb_diff_th_varres
