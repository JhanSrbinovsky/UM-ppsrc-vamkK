...
! No External Routines:
      ...
      IF (lhook) CALL dr_hook('H_DIFF_U',zhook_in,zhook_handle)
...
!$OMP& SHARED(off_x, off_y, rows, row_length, field, u, r_at_u,         &
...
!$OMP& SHARED(level_flat, level_base, rows, row_length, ground, r_at_u, &
!$OMP&        mask_i, mask_j)
      Do ka = 1, level_flat - 1
        ...
        Do j = 1, rows
          Do i = 0, row_length
            ...
            End If
            If (r_at_u(i+1,j,k)  <   r_at_u(i,j,k+1) .AND.              &
        ...
        Do j = 0, rows
          Do i = 1, row_length
            ...
            Else
              lower_surface=r_at_u(i,j,k-1)
            End If
            If (r_at_u(i,j+1,k)  <   r_at_u(i,j,k+1) .AND.              &
...
!$OMP  PARALLEL DO DEFAULT(NONE) SCHEDULE(STATIC)                       &
!$OMP& PRIVATE(k,j,i)                                                   &
!$OMP& SHARED(order, active_diffusion_order, j0, j1, row_length,        &
...
!$OMP& SHARED(n_active_levels, indx, diffusion_order, row_length,       &
!$OMP&        R_u, field, j0, j1)
      Do k = 1, n_active_levels
        kk = indx(k)
        ...
        Do j = j0, j1
          Do i = 1, row_length
            R_u(i,j,kk) = R_u(i,j,kk) + field(i,j,k) * sign
          End Do
        End Do
      End Do
!$OMP END PARALLEL DO
...
! End of routine
      IF (lhook) CALL dr_hook('H_DIFF_U',zhook_out,zhook_handle)
      RETURN
