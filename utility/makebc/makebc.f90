...
! Subroutine Interface :
      SUBROUTINE makebc(um_versn,                                       &
                        tracers_active,                                 &
                        no_tracers,tr_ukca_active,no_tr_ukca,           &
...
!     Reinitialising of boundary dataset not supported yet.
      type_letter_1(unit_no_bc:unit_no_bc+max_n_intf_a-1) = 'b'
      ft_steps     (unit_no_bc:unit_no_bc+max_n_intf_a-1) = 0
      ft_firststep (unit_no_bc:unit_no_bc+max_n_intf_a-1) = 0
      ft_output    (unit_no_bc:unit_no_bc+max_n_intf_a-1) = 'N'
...
!     Print out INTFCNTL namelist variables (Read in intf_ctl)
      IF(printstatus >= prstatus_diag)THEN
        WRITE (6,'(A)') ' '
        WRITE (6,'(A)') ' Namelist INTFCNSTA read in'
        DO jintf=1,n_intf_a
          WRITE (6,'(A)')       ' '
          WRITE (6,'(A,I3)')    ' For area ',jintf
          WRITE (6,'(A,I3)')    ' a_intf_start_hr  ',a_intf_start_hr(jintf)
          WRITE (6,'(A,I3)')    ' a_intf_freq_hr   ',a_intf_freq_hr(jintf)
          WRITE (6,'(A,I3)')    ' a_intf_end_hr    ',a_intf_end_hr(jintf)
          WRITE (6,'(A,I5)')    ' intf_p_rows      ',intf_p_rows(jintf)
          WRITE (6,'(A,I5)')    ' intf_row_length  ',intf_row_length(jintf)
          WRITE (6,'(A,I3)')    ' intf_p_levels    ',intf_p_levels(jintf)
          WRITE (6,'(A,I3)')    ' intf_q_levels    ',intf_q_levels(jintf)
          WRITE (6,'(A,I3)')    ' intf_tr_levels   ',intf_tr_levels(jintf)
          WRITE (6,'(A,F5.12)') ' intf_firstlat    ',intf_firstlat(jintf)
          WRITE (6,'(A,F5.12)') ' intf_firstlong   ',intf_firstlong(jintf)
          WRITE (6,'(A,F5.12)') ' intf_nsspace     ',intf_nsspace(jintf)
          WRITE (6,'(A,F5.12)') ' intf_ewspace     ',intf_ewspace(jintf)
          WRITE (6,'(A,F5.12)') ' intf_polelat     ',intf_polelat(jintf)
          WRITE (6,'(A,F5.12)') ' intf_polelong    ',intf_polelong(jintf)
          WRITE (6,'(A,I2)')    ' intf_pack        ',intf_pack(jintf)
          WRITE (6,'(A,I2)')    ' intfwidtha       ',intfwidtha(jintf)
          WRITE (6,'(A,I2)')    ' intf_ExtHalo_NS  ',intf_exthalo_ns(jintf)
          WRITE (6,'(A,I2)')    ' intf_ExtHalo_EW  ',intf_exthalo_ew(jintf)
          WRITE (6,'(A,L7)')    ' intf_vert_interp ',intf_vert_interp(jintf)
          WRITE (6,'(A,L7)')    ' intf_avoid_rot   ',intf_avoid_rot(jintf)
          WRITE (6,'(A,L7)')    ' intf_l_eg_out    ',intf_l_eg_out(jintf)
        END DO
        WRITE(6,'(A)') ' '
      END IF
...
! DEPENDS ON: loop_over_dumps
       CALL loop_over_dumps (unit_no_bc,um_versn,        &
! ARGINFA Headers for atmosphere interface data sets
        fixhd_intfa, inthd_intfa, lookup_intfa,                         &
        realhd_intfa,levdepc_intfa,                                     &
      ! Row/Col DEPC for variable resolution LBCs
