...
! Note, this copy is for the new 2 bin dust scheme. This can be
! removed and the routine simplified when the mode outputs beta_dust
! directly from STASH.

SUBROUTINE DUST_VIS2(vis_incppn,    & !in
                     tscreen,       & !in
                     pstar,         & !in
                     dust1,         & !in
                     dust2,         & !in
...
! Local Constants:
CHARACTER(LEN=*), PARAMETER :: RoutineName = "DUST_VIS2"
...
REAL :: beta_air
! specific extinction coefficients at 550nm for each bin (2 bin scheme)
REAL, PARAMETER, DIMENSION(2)  :: k_ext=(/ 700.367, 141.453/)
! air density
REAL :: rho
...
DO j = 1, vis_tot % Hdr % NumRows
  DO i = 1, vis_tot % Hdr % NumCols
    ...
    beta_dust(i,j)= rho * (                                      &
            dust1%RData(i,j)*k_ext(1) + dust2%RData(i,j)*k_ext(2))
  END DO
END DO
